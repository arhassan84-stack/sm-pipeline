...
Changes vs pt_wavenet_multidt_noise_gpu.py:
  - Traces cropped to WINDOW_MS ms centered on the middle of the full trace
  - Features: only 6 raw stats from the cropped window (no 906 cached features)
  - n_abs defined on full-trace max (same physics / same label distribution)
  - POOL010 = WINDOW_MS // 256  →  all windows yield ~640 bins entering WaveNet
  - AUG_SHIFT scaled to ~3% of window in dt100 bins

Usage:  python pt_wavenet_noise_window_gpu.py --window_ms 512
...
# ── Hyper-parameters ───────────────────────────────────────────────────────────
LABEL        = f'wavenet_noise_w{WINDOW_MS}'
CHANNELS     = 256
BATCH_SIZE   = 128
DL_WORKERS   = 2
MAX_EPOCHS   = 200
...
N_SCALE        = 1e3      # predict n in kHz for numerical stability
...
class WindowedNoiseFCSDataset(Dataset):
    ...
    n_abs  = n_frac * max(full_dt010 * scale)   [labels consistent with full model]
    ...
    def __init__(self, i010, i010_idx, i050, i100,
        ...
        self.i010     = i010         # (N_all, 40960) full train mmap/array
        self.i010_idx = i010_idx     # (N_tr,) global row indices into i010
        self.i050     = i050         # (N_tr, 8192)
        self.i100     = i100         # (N_tr, 4096)
    ...
    def __getitem__(self, idx):
        t010_full = self.i010[self.i010_idx[idx]]   # (40960,)
        t050_full = self.i050[idx]                   # (8192,)
        t100_full = self.i100[idx]                   # (4096,)
        ...
        # 2. Background level from full-trace max
        ...
        bg_counts = self.rng.poisson(n_abs * DT_MIN_S, size=40960).astype(np.float32)
        bg_010    = bg_counts / DT_MIN_S
        ...
        t010n = (t010s + bg_010)[S010:S010 + W010]     # (W010,)
        t050n = (t050s + bg_050)[S050:S050 + W050]     # (W050,)
        t100n = (t100s + bg_100)[S100:S100 + W100]     # (W100,)
...
t_total = time.time()
log(f"Device: {device}  |  Window: {WINDOW_MS} ms")
...
rs_010  = i010_tr[tr_global[tr_idx[rs_samp]], S010:S010 + W010]  # (5k, W010)
...
log(f"  raw_stat_mean: {np.round(raw_stat_mean, 3)}")
...
i010_val_full = i010_tr[tr_global[val_idx]]            # (N_val, 40960)
n_abs_val     = (n_frac_val * i010_val_full.max(axis=1)).astype(np.float32)
...
def predict_n(i010_all, i010_idx, i050_full, i100_full, rng_seed):
    """Add fixed-noise augmentation, crop to window, predict n_abs.

    Memory-efficient: n_abs computed in chunks (avoids N×40960 temp array);
    i010 normalized in-place in the loop (avoids a second N×W010 copy).
    """
    N   = len(i050_full)
    ...
    CHUNK     = 256
    # Compute n_abs in chunks to avoid materialising N×40960 array (~37 GB for train)
    n_abs     = np.empty(N, dtype=np.float32)
    for s in range(0, N, CHUNK):
        e = min(s + CHUNK, N)
        n_abs[s:e] = n_frac[s:e] * i010_all[i010_idx[s:e]].max(axis=1)

    i010_n    = np.empty((N, W010), dtype=np.float32)  # normalized in-place (no 2nd copy)
