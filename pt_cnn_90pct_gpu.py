"""
Improvement: CNN fusion trained on 90% split (90k train traces + features).
...
# Save weights + scaler for ensemble
torch.save(model.state_dict(), 'model_cnn_90pct.pt')
np.save('scaler_cnn90pct_mean.npy', feat_scaler.mean_)
np.save('scaler_cnn90pct_scale.npy', feat_scaler.scale_)
log("Saved model_cnn_90pct.pt + scaler")
...
summary = (f"Task: {LABEL}\nArchitecture: CNN (90k traces) + FC (207 features) → fusion head\n"
...
ax=axes[0]; ax.scatter(d_test,d_pred_te,alpha=0.2,s=5,color='seagreen')
