...
PIPELINE_VERSION = 'v3.95'   # hardcoded — do not import from version.py
...
def make_event_plot_3panel(stem, ch_id, t_peak, i100, baseline,
    ...
    # Edge baseline for the middle panel: minimum of the two event window edges
    # (t_left/t_right), clamped to trace bounds.  The minimum is used (rather
    # than the mean) so that a noise-model baseline contaminated by a slow
    # transit on one side does not raise the subtraction level above the true
    # pre-event background — matching the logic in _edge_baseline in stage3.
    ...
    seg_bl  = np.maximum(0.0, seg - edge_bl)    # edge-baseline-subtracted, clipped
