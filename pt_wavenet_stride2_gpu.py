"""
High-resolution WaveNet: stride=2 input projection (4096→2048) with 10 dilated blocks.

Motivation: The standard WaveNet uses stride=4 (4096→1024), which coarsens the ACF
signal before it reaches the dilated blocks.  With stride=2 (4096→2048), each dilated
block operates at 2× finer temporal resolution, better capturing short-lag structure
critical for fast diffusion (d>=1) where the ACF decays within ~100-200 original lags.

Using 10 dilated blocks (vs 8) to maintain full-trace receptive field coverage:
  dilations = [1, 2, 4, 8, 16, 32, 64, 128, 256, 512]
  RF = 2*(1+2+4+8+16+32+64+128+256+512) * 2 = 4092 original lags  (covers full 4096-lag trace)

vs original WaveNet:
  stride=4, 8 blocks  →  RF = 2040 original lags at 4-lag resolution
  stride=2, 10 blocks →  RF = 4092 original lags at 2-lag resolution  ← this model

Saves pred_logd_wavenet_stride2_train/test.npy for meta-learner.
"""
...
WEIGHT_DECAY = 1e-4
T0           = 30
DILATIONS    = [1, 2, 4, 8, 16, 32, 64, 128, 256, 512]   # 10 blocks
STRIDE       = 2    # 4096 → 2048 (2× finer than standard stride=4)
...
class WaveNet1D(nn.Module):
    def __init__(self, feat_dim, channels=128, dilations=None, stride=2):
        super().__init__()
        if dilations is None:
            dilations = [1, 2, 4, 8, 16, 32, 64, 128, 256, 512]

        # stride=2: 4096 → 2048  (kernel=8, padding=3 preserves exact length)
        self.input_proj = nn.Sequential(
            nn.Conv1d(1, channels, kernel_size=8, stride=stride, padding=3),
...
model = WaveNet1D(feat_dim=X_tr.shape[1], channels=CHANNELS, dilations=DILATIONS, stride=STRIDE).to(device)
...
rf_original = rf_reduced * STRIDE
log(f"Training {LABEL}  params={n_params:,}")
log(f"  stride={STRIDE}  channels={CHANNELS}  dilations={DILATIONS}")
log(f"  Sequence length after proj: {4096 // STRIDE} steps  (2× finer than stride=4)")
...
torch.save(model.state_dict(), 'model_wavenet_stride2.pt')
np.save('scaler_wavenet_stride2_mean.npy', feat_scaler.mean_)
np.save('scaler_wavenet_stride2_scale.npy', feat_scaler.scale_)
log("Saved model_wavenet_stride2.pt + scaler")
...
np.save('pred_logd_wavenet_stride2_train.npy', logd_pred_tr)
np.save('pred_logd_wavenet_stride2_test.npy',  logd_pred_te)
log("Saved pred_logd_wavenet_stride2_train/test.npy")
...
summary = (
    f"Task: {LABEL}\n"
    f"Architecture: WaveNet1D stride={STRIDE} channels={CHANNELS} dilations={DILATIONS}\n"
    f"  Sequence length after proj: {4096//STRIDE} (2x finer than stride=4)\n"
