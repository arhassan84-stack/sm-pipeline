"""
WaveNet specialist for fast diffusers (d >= 0.8).

Same WaveNet1D(channels=256) architecture as wavenet_wide_aug.
Trained on augmented cache filtered to d >= 0.8 (~230k of ~390k samples)
to focus capacity on the fast-diffuser regime that is the main bottleneck.

Boundary overlap at d=0.8 is intentional so the specialist covers the
transition zone around the d=1 threshold.

Predicts on the FULL 9502-sample test set (d<=10 mask, NOT filtered to d>=0.5)
so the router can compare predictions for every test sample.

Saves:
  model_wavenet_wide_specialist.pt
  scaler_wavenet_wide_specialist_mean/scale.npy
  pred_logd_wavenet_wide_specialist_test.npy   (9502 rows, full test set)
  results_pt_wavenet_wide_specialist.txt
"""
...
LABEL        = 'pt_wavenet_wide_specialist'
CHANNELS     = 256
BATCH_SIZE   = 128
EVAL_BATCH   = 256
MAX_EPOCHS   = 200
...
D_THRESHOLD  = 0.8   # train only on d >= this value (fast-diffuser regime)
...
log(f"Raw aug train: {len(d_train)}")

# Filter to fast-diffuser regime (d >= D_THRESHOLD, also cap at d<=10)
mask_fast = (d_train >= D_THRESHOLD) & (d_train <= 10)
i_train, X_train, d_train = i_train[mask_fast], X_train[mask_fast], d_train[mask_fast]
log(f"After d>={D_THRESHOLD} & d<=10 filter: {len(d_train)} training samples")

# ── Load FULL test set (d<=10 mask, all 9502 samples) ─────────────────────────
...
log(f"  channels={CHANNELS}  dilations={DILATIONS}  D_threshold={D_THRESHOLD}")
log(f"  RF={rf_original} original lags  train={len(tr_idx)}  val={len(val_idx)}")
...
# ── Save model + scaler ───────────────────────────────────────────────────────
torch.save(model.state_dict(), 'model_wavenet_wide_specialist.pt')
np.save('scaler_wavenet_wide_specialist_mean.npy',  feat_scaler.mean_)
np.save('scaler_wavenet_wide_specialist_scale.npy', feat_scaler.scale_)
log("Saved model_wavenet_wide_specialist.pt + scaler")

# ── Predict on FULL test set (9502 samples) ───────────────────────────────────
model.eval()
with torch.no_grad():
...
np.save('pred_logd_wavenet_wide_specialist_test.npy', logd_pred_te)
log("Saved pred_logd_wavenet_wide_specialist_test.npy")
...
summary = (
    ...
    f"Params: {n_params:,}\n"
    f"Training filter: d>={D_THRESHOLD}  N_train={len(d_train)}\n"
...
print(f"\n{'='*60}\n{summary}{'='*60}")
with open('results_pt_wavenet_wide_specialist.txt', 'w') as f:
    f.write(summary)
log("Saved results_pt_wavenet_wide_specialist.txt"); print("Done.")
