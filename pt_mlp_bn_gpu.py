"""
Phase 3 — PyTorch MLP with Batch Normalization + Dropout + AdamW + LR scheduling.
Architecture: 207 → 2048 → 1024 → 512 → 256 → 128 → 1
...
y_tr  = torch.tensor(log_d_train[tr_idx].astype(np.float32)).to(device)
X_val = torch.tensor(X_tr_sc[val_idx]).to(device)
y_val = torch.tensor(log_d_train[val_idx].astype(np.float32)).to(device)
X_te  = torch.tensor(X_te_sc).to(device)

train_loader = DataLoader(TensorDataset(X_tr, y_tr), batch_size=BATCH_SIZE, shuffle=True)

model     = MLP_BN(in_dim=X_tr.shape[1], dropout=DROPOUT).to(device)
...
with torch.no_grad():
    d_pred_train = np.exp(model(X_tr_all).cpu().numpy())
    d_pred_test  = np.exp(model(X_te).cpu().numpy())
...
summary = (
    f"Task: {LABEL}\n"
    f"Architecture: 2048-1024-512-256-128 + BN + Dropout({DROPOUT})\n"
...
ax = axes[0]
ax.scatter(d_test, d_pred_test, alpha=0.2, s=5, color='steelblue')
