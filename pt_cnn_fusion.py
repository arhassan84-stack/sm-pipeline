...
Branch A: 1D CNN processes raw 4096-length intensity traces
  Conv layers extract local temporal patterns at multiple scales.
Branch B: FC sub-network processes the 207 engineered features.
Final: concatenate both → shared head → scalar prediction.
"""
import numpy as np
import time
import torch
...
class CNN_Branch(nn.Module):
    """1D CNN for raw 4096-length trace → 256-dim embedding."""
    def __init__(self):
        super().__init__()
        self.net = nn.Sequential(
            # Input: (B, 1, 4096)
            nn.Conv1d(1, 32, kernel_size=16, stride=4, padding=6),  # → (B,32,1024)
            nn.BatchNorm1d(32), nn.ReLU(),
            nn.Conv1d(32, 64, kernel_size=8, stride=4, padding=2),  # → (B,64,256)
            nn.BatchNorm1d(64), nn.ReLU(),
            nn.Conv1d(64, 128, kernel_size=4, stride=2, padding=1), # → (B,128,128)
            nn.BatchNorm1d(128), nn.ReLU(),
            nn.Conv1d(128, 256, kernel_size=4, stride=2, padding=1),# → (B,256,64)
            nn.BatchNorm1d(256), nn.ReLU(),
            nn.AdaptiveAvgPool1d(1),                                  # → (B,256,1)
        )

    def forward(self, x):
        return self.net(x.unsqueeze(1)).squeeze(2)   # (B, 256)


class Feature_Branch(nn.Module):
    """FC sub-network for 207 engineered features → 256-dim embedding."""
...
class FusionNet(nn.Module):
    ...
    def forward(self, trace, feats):
        cnn_emb  = self.cnn_branch(trace)    # (B, 256)
        feat_emb = self.feat_branch(feats)   # (B, 256)
        combined = torch.cat([cnn_emb, feat_emb], dim=1)  # (B, 512)
...
# Raw intensity traces (50k, 4096) — use 50% cache (matches 90% cache indices aren't stored)
# We load the 90% feature cache to get d labels, but use 50% raw traces
# Both 50k trace cache and 90% feature cache share the same first 50k train indices
i_train_50k = np.load('cache_i_train.npy').astype(np.float32)   # (50000, 4096)
X_train_50k = np.load('cache_X_train_psd.npy').astype(np.float32)  # (50000, 207)
d_train_50k = np.load('cache_d_train.npy')

i_test_5k   = np.load('cache_i_test.npy').astype(np.float32)    # (5000, 4096)
X_test_5k   = np.load('cache_X_test_psd.npy').astype(np.float32)   # (5000, 207)
...
# Normalize raw traces per-sample: zero mean, unit variance
i_mean = i_train.mean(axis=1, keepdims=True)
i_std  = i_train.std(axis=1, keepdims=True) + 1e-8
i_train_n = (i_train - i_mean) / i_std

i_mean_te = i_test.mean(axis=1, keepdims=True)
i_std_te  = i_test.std(axis=1, keepdims=True) + 1e-8
i_test_n  = (i_test - i_mean_te) / i_std_te
...
i_tr   = torch.tensor(i_train_n[tr_idx])
X_tr   = torch.tensor(X_tr_sc[tr_idx])
y_tr   = torch.tensor(log_d_train[tr_idx])
i_val  = torch.tensor(i_train_n[val_idx])
X_val  = torch.tensor(X_tr_sc[val_idx])
y_val  = torch.tensor(log_d_train[val_idx])
i_te   = torch.tensor(i_test_n)
X_te   = torch.tensor(X_te_sc)

train_loader = DataLoader(
    TensorDataset(i_tr, X_tr, y_tr), batch_size=BATCH_SIZE, shuffle=True)

model     = FusionNet(feat_dim=X_tr.shape[1])
...
model.eval()
with torch.no_grad():
    d_pred_train = np.exp(model(torch.tensor(i_train_n), torch.tensor(X_tr_sc)).numpy())
    d_pred_test  = np.exp(model(i_te, X_te).numpy())
...
summary = (
    ...
    f"CNN: Conv1d 1→32→64→128→256, AdaptiveAvgPool → 256-dim\n"
    f"Feat: FC 207→512→256\n"
    f"Head: 512→256→64→1\n"
