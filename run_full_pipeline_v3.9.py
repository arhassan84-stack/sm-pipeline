...
PIPELINE_VERSION = 'v3.9'   # hardcoded — do not import from version.py
...
def make_event_plot_3panel(stem, ch_id, t_peak, i100, baseline,
    ...
    # Edge baseline for the middle panel: use the final event window edges
    # (t_left/t_right), clamped to trace bounds.  This matches the constant
    # background level used during D inference so the subtracted trace is
    # physically consistent with the model input.
    tl_eff  = max(0, min(t_left,  N - 1)) if t_left  is not None else t0
    tr_eff  = max(0, min(t_right, N - 1)) if t_right is not None else t1
    edge_bl = float(0.5 * (baseline[tl_eff] + baseline[tr_eff]))
