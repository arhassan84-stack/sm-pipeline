"""
Build train/test feature caches for the multi-dt FCS simulation dataset.

Loads sims_multidt_noise0_i_{dt010,dt050,dt100}.npy and sims_multidt_noise0_d.npy,
...
Output files  (30 total):
  cache_i_{train,test}_multidt_dt010.npy    (N × 40960) float32 raw traces
  cache_X_{train,test}_multidt_dt010.npy    (N × 302)   float64 features
  cache_d_{train,test}_multidt_dt010.npy    (N,)        float64 D values
  ... same for dt050  (N × 8192)
  ... same for dt100  (N × 4096)

Usage:
  python build_cache_multidt.py --workers 8 --chunk-size 500
"""
import numpy as np
import argparse
import time
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Build multi-dt feature caches')
    ...
    parser.add_argument('--tag',        type=str, default='multidt',
                        help='Dataset tag prefix (default: multidt)')
    args = parser.parse_args()

    t_total = time.time()
    DT_CONFIGS = [
        ('dt010', 'sims_multidt_noise0_i_dt010.npy', 40960),
        ('dt050', 'sims_multidt_noise0_i_dt050.npy',  8192),
        ('dt100', 'sims_multidt_noise0_i_dt100.npy',  4096),
    ]

    # ── One shared train/test split ───────────────────────────────────────────
    log("Creating shared train/test split from D values...")
    d_all   = np.load('sims_multidt_noise0_d.npy')
    N_total = len(d_all)
