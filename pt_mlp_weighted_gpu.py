"""
MLP+BN+Dropout with CosineAnnealingWarmRestarts + weighted MSE loss.
Weight = FAST_WEIGHT for d>=1 samples, 1.0 for d<1.
Saves weights overwriting model_mlp_cosine.pt (same ensemble entry point).
"""
...
# Per-sample weights: FAST_WEIGHT for d>=1, 1.0 for d<1
...
# Save overwriting the cosine model slots (used by ensemble)
torch.save(model.state_dict(), 'model_mlp_cosine.pt')
np.save('scaler_mlp_cosine_mean.npy', scaler.mean_)
np.save('scaler_mlp_cosine_scale.npy', scaler.scale_)
log("Saved model_mlp_cosine.pt + scaler (overwriting for ensemble)")
