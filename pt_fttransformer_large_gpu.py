"""
FT-Transformer LARGE (E=128, 6 layers, ~1.75M params).

Architecture change vs original (E=64, 4 layers, 236k params):
  - Embedding dim doubled: E=128 (richer feature representations)
  - Depth increased: 6 transformer layers (more composition of interactions)
  - Feedforward dim doubled: dim_ff=512
  - Slightly higher dropout (0.15) and lower LR (5e-4) to compensate for larger capacity

Saves model_fttransformer_large.pt for ensemble use.
"""
...
class FTTransformer(nn.Module):
    def __init__(self, n_features, E=128, n_heads=8, n_layers=6,
                 dim_ff=512, dropout=0.15):
        super().__init__()
...
torch.save(model.state_dict(), 'model_fttransformer_large.pt')
np.save('scaler_fttransformer_large_mean.npy', scaler.mean_)
np.save('scaler_fttransformer_large_scale.npy', scaler.scale_)
log("Saved model_fttransformer_large.pt + scaler")
