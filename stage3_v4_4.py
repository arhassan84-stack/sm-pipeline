"""
stage3_v4_4.py — Stage 3 event detection pipeline, version 4.4
═════════════════════════════════════════════════════════════

Changes vs stage3_v4_3:

1. Transit-time stopping criterion replaces the window-width criterion.
   Previously the expansion loop stopped when the proposed window width
   exceeded tau_max (looked up from D̂).  Now it stops when the transit
   time computed on the proposed window exceeds tau_max.

   Transit time definition (TRANSIT_CUTOFF = 0.40):
       cutoff_I  = edge_bl + 0.40 × (max(signal[ts_new:te_new]) − edge_bl)
       transit   = Σ (signal[ts_new:te_new] > cutoff_I)  × 1 ms

   The edge baseline used is min(baseline[ts_new], baseline[te_new]),
   i.e. it updates with each proposed window (Option A).

All other logic (BL_FACTORS, pearson_fn callback, pearson_best_factor)
is unchanged from v4.3.
"""
...
# ── v4.4: transit-time stopping criterion ─────────────────────────────────────
TRANSIT_CUTOFF = 0.40   # fraction of (peak − baseline) above which bins are counted
...
def _transit_time(signal: np.ndarray, t_left: int, t_right: int,
                  baseline: float) -> int:
    """
    Number of 1-ms bins in signal[t_left:t_right+1] whose intensity exceeds
        baseline + TRANSIT_CUTOFF × (peak − baseline)
    where peak = max(signal[t_left:t_right+1]).
    Returns 0 if peak ≤ baseline.
    """
    seg    = signal[t_left : t_right + 1]
    peak   = float(seg.max())
    excess = peak - baseline
    if excess <= 0.0:
        return 0
    thresh = baseline + TRANSIT_CUTOFF * excess
    return int(np.sum(seg > thresh))
    ...
    for i, (ev, D_hat_init) in enumerate(zip(seed_events, initial_d_hats)):
        ...
        if allow_swallow and ev['id'] in swallowed_ids:
            ...
            results.append(dict(
                ...
                pearson_best_factor = 1.0,   # no expansion step — Pearson not applied
            ))
            continue
        ...
        # v4.2: Pearson-guided stopping criterion at the seed step
        pearson_best_factor = 1.0
        if pearson_fn is not None:
            _fdict = {f: float(factor_initial_d[f][i]) for f in BL_FACTORS}
            _pd, _pf, _ = pearson_fn(tp, _fdict)
            D_hat_stop = float(_pd)
            pearson_best_factor = float(_pf)
        else:
            D_hat_stop = D_hat_init  # factor=1.0 (unchanged from v4.1)

        fast_group = False if category == 1 else (D_hat_stop > D_FAST_CUTOFF)
        ...
        if not fast_group:
            tau_max = _lookup_tau_max(D_hat_stop)
            ...
            while w < W_MAX_MS:
                ...
                if category != 1:
                    bl_proposed  = float(min(bundle.baseline[ts_new],
                                             bundle.baseline[te_new]))
                    transit_ms   = _transit_time(bundle.i1ms, ts_new, te_new,
                                                 bl_proposed)
                    if transit_ms > tau_max:
                        break
                ...
                # v4.2: Pearson-guided stopping criterion at this expansion step
                if pearson_fn is not None:
                    _fdict = {f: factor_d_hist[f][-1] for f in BL_FACTORS}
                    _pd, _pf, _ = pearson_fn(tp, _fdict)
                    D_hat_stop = float(_pd)
                    pearson_best_factor = float(_pf)
                else:
                    D_hat_stop = D_hat  # factor=1.0 (unchanged from v4.1)

                if category != 1:
                    if D_hat_stop > D_FAST_CUTOFF:
                        fast_group = True
                        break
                    tau_max = _lookup_tau_max(D_hat_stop)
        ...
        results.append(dict(
            ...
            d_hat_by_factor     = {f: factor_d_hist[f][-1] for f in BL_FACTORS},
            d_hist_by_factor    = factor_d_hist,
            pearson_best_factor = pearson_best_factor,  # v4.2: winning BL_FACTOR from final Pearson step
        ))
...
def run_stage3_v2(bundle: TraceBundle, d_model, radius_cap: int = 128,
    ...
    all_cat1 = _step2(bundle, cat1, owner1,   d_model, category=1, allow_swallow=True, pearson_fn=pearson_fn)
    all_cat2 = _step2(bundle, cat2, claimed2, d_model, category=2, pearson_fn=pearson_fn)
