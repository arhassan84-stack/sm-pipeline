"""
Multi-dt WaveNet — Option C: Shared-weight branches.

One WaveNet1D backbone is called 3 times (once per dt value at native resolution).
Shared weights force the network to learn dt-agnostic temporal features.
The 3 embeddings (768-dim total) + features (906-dim) → shared head.

Label: wavenet_multidt_C
"""
...
class WaveNetBackbone(nn.Module):
    ...
    def forward(self, trace):
        ...
        x = self.blocks(x)
        return self.gap(x).squeeze(2)


class WaveNetMultiDT_C(nn.Module):
    """One shared WaveNet backbone called for each of the 3 dt traces."""
    def __init__(self, feat_dim, channels=256, dilations=None):
        super().__init__()
        self.backbone = WaveNetBackbone(channels, dilations)
    ...
    def forward(self, t010, t050, t100, feats):
        e010 = self.backbone(t010)
        e050 = self.backbone(t050)
        e100 = self.backbone(t100)
...
i010_val_n = norm_traces(i010_tr[val_idx])
...
model = WaveNetMultiDT_C(feat_dim=feat_dim, channels=CHANNELS, dilations=DILATIONS).to(device)
opt   = torch.optim.AdamW(model.parameters(), lr=LR, weight_decay=WEIGHT_DECAY)
sched = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(opt, T_0=T0, T_mult=1)
crit  = nn.MSELoss()

n_params = sum(p.numel() for p in model.parameters())
log(f"Training {LABEL}  params={n_params:,}  (shared backbone, 3 passes)")
...
model.eval()
i010_te_n = norm_traces(i010_te); i050_te_n = norm_traces(i050_te); i100_te_n = norm_traces(i100_te)
i010_tr_n = norm_traces(i010_tr); i050_tr_n = norm_traces(i050_tr); i100_tr_n = norm_traces(i100_tr)

with torch.no_grad():
    logd_pred_te = batch_predict(lambda a, b, c, x: model(a, b, c, x),
                                 torch.tensor(i010_te_n), torch.tensor(i050_te_n),
                                 torch.tensor(i100_te_n), torch.tensor(X_te_sc),
                                 bs=EVAL_BATCH).numpy()
    logd_pred_tr = batch_predict(lambda a, b, c, x: model(a, b, c, x),
                                 torch.tensor(i010_tr_n), torch.tensor(i050_tr_n),
                                 torch.tensor(i100_tr_n), torch.tensor(X_tr_sc),
                                 bs=EVAL_BATCH).numpy()
...
summary = (
    f"Task: {LABEL}\n"
    f"Architecture: shared WaveNet backbone (3 passes, dt010/050/100 native res) + 906-dim features\n"
    f"  WaveNetMultiDT_C channels={CHANNELS} dilations={DILATIONS}\n"
    f"Params: {n_params:,}\n"
