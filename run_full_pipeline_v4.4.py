...
Changes vs v4.35:
  - stage3_module switched to stage3_v4_4, which replaces the
    window-width expansion stopping criterion with a transit-time
    criterion: expansion stops when the number of bins in the proposed
    window [ts_new, te_new] that exceed
        baseline + 0.40 × (peak − baseline)
    surpasses tau_max (looked up from D̂).
    Baseline updates with each candidate window (edge baseline).
...
PIPELINE_VERSION = 'v4.4'   # hardcoded — do not import from version.py
