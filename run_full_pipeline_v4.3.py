...
Changes vs v4.2:
  - Pearson-guided expansion loop: a pearson_fn callback is built per channel
    (closing over the sim library) and injected into stage3_v4_2's _step2.
    At each expansion step, Pearson correlation is computed for all 6 BL_FACTORS
    and the factor with the highest score determines tau_max and the D_FAST_CUTOFF
    stopping check instead of the fixed factor=1.0 prediction.
  - stage3_module changed to 'stage3_v4_2' which implements the callback interface.
  - Events gain 'pearson_best_factor' (recorded by stage3_v4_2) in the CSV.
  - Post-hoc Pearson highlighting (gold plots, metadata.csv) unchanged from v4.2.

For each measurement (one loop):
  1. run_pipeline (via stage3_v4_2 + pearson_fn)  → {stem}_{ch}_events.csv
...
PIPELINE_VERSION = 'v4.3'   # hardcoded — do not import from version.py

# v4.0 baseline subtraction factors (must match stage3_v4_2.BL_FACTORS)
            ...
            for ev in events:
                ...
                for factor, (chosen, D_sim_val, pearson, sim_raw) in factor_results.items():
                    ...
                    all_meta_rows.append({
                        ...
                        'pearson_best_factor_stage3': stage3_best_factor,  # v4.3
                    })
