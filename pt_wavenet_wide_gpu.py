"""
Wide WaveNet: channels=256 (doubled capacity) for FCS diffusion prediction.

Motivation: The standard WaveNet uses channels=128 (~580k params). Doubling to 256
increases representational capacity in each dilated residual block, potentially
reducing bias in the complex d=[3,10] fast-diffusion regime.

Architecture: identical structure to pt_wavenet, channels=128→256
  - stride-4 input projection: Conv1d(1→256, k=16, stride=4) → 1024 steps
  - 8 DilatedResBlocks(256, d) for d in [1,2,4,8,16,32,64,128]
  - GlobalAvgPool → 256-dim trace embedding
  - Feature branch: 283f → 256 → 128
  - Fusion head: [256+128=384] → 256 → 64 → 1

Params: ~1.8M (vs 580k for channels=128)
RF: 2040 original lags (same as original WaveNet)

Saves pred_logd_wavenet_wide_train/test.npy for meta-learner.
"""
...
LABEL        = 'pt_wavenet_wide'
CHANNELS     = 256      # doubled from standard 128
BATCH_SIZE   = 128      # reduced slightly to accommodate larger model
EVAL_BATCH   = 256
MAX_EPOCHS   = 200
PATIENCE     = 25
LR           = 5e-4
...
log(f"  channels={CHANNELS} (2× standard)  dilations={DILATIONS}  RF={rf_original} original lags")
...
torch.save(model.state_dict(), 'model_wavenet_wide.pt')
np.save('scaler_wavenet_wide_mean.npy', feat_scaler.mean_)
np.save('scaler_wavenet_wide_scale.npy', feat_scaler.scale_)
log("Saved model_wavenet_wide.pt + scaler")
...
np.save('pred_logd_wavenet_wide_train.npy', logd_pred_tr)
np.save('pred_logd_wavenet_wide_test.npy',  logd_pred_te)
log("Saved pred_logd_wavenet_wide_train/test.npy")
