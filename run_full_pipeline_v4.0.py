...
Changes vs v3.95:
  - stage3_v4: per-factor D predictions (BL_FACTORS = [1.05, 1.10, 1.15, 1.20])
  - Channel-aware sim library: S1 → GFP sims, S2 → mCherry2 sims
  - 4 event plots per event (one per factor); suptitle shows factor value
  - Middle panel uses factor * edge_bl for baseline subtraction
  - Bottom panel Pearson-matched sim uses factor-specific D prediction
...
PIPELINE_VERSION = 'v4.0'   # hardcoded — do not import from version.py

# v4.0 baseline subtraction factors (must match stage3_v4.BL_FACTORS)
BL_FACTORS = [1.05, 1.10, 1.15, 1.20]
...
def make_event_plot_3panel(stem, ch_id, t_peak, i100, baseline,
    ...
    fig.suptitle(f'factor = {factor:.2f}', fontsize=FS_SUPTITLE, color='black')
...
try:
    ...
    with open(meta_path, 'w', newline='') as f:
        writer = csv.DictWriter(
            f, fieldnames=['stem', 'ch_id', 't_peak_ms', 'factor', 'D_hat', 'D_sim', 'sim_idx'])
        writer.writeheader()
