"""
Build train/test feature caches for the multi-dt4 FCS simulation dataset.

Loads sims_multidt4_noise0_i_{dt005,dt010,dt050,dt100}.npy and sims_multidt4_noise0_d.npy,
...
Output files  (30 total):
  cache_i_{train,test}_multidt4_dt005.npy    (N × 81920) float32 raw traces
  cache_X_{train,test}_multidt4_dt005.npy    (N × 302)   float64 features
  cache_d_{train,test}_multidt4_dt005.npy    (N,)        float64 D values
  ... same for dt010  (N × 40960)
  ... same for dt050  (N × 8192)
  ... same for dt100  (N × 4096)

Usage:
  python build_cache_multidt4.py --workers 8 --chunk-size 500
"""
import numpy as np
import argparse
import time
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Build multi-dt4 feature caches')
    ...
    parser.add_argument('--tag',        type=str, default='multidt4',
                        help='Dataset tag prefix (default: multidt4)')
    args = parser.parse_args()

    t_total = time.time()
    DT_CONFIGS = [
        ('dt005', 'sims_multidt4_noise0_i_dt005.npy', 81920),  # NEW: dt=0.05ms
        ('dt010', 'sims_multidt4_noise0_i_dt010.npy', 40960),
        ('dt050', 'sims_multidt4_noise0_i_dt050.npy',  8192),
        ('dt100', 'sims_multidt4_noise0_i_dt100.npy',  4096),
    ]

    # ── One shared train/test split ───────────────────────────────────────────
    log("Creating shared train/test split from D values...")
    d_all   = np.load(f'sims_multidt4_noise0_d.npy')
    N_total = len(d_all)
    ...
    for dt_tag, i_file, expected_bins in DT_CONFIGS:
        ...
        # Write split arrays to disk in chunks via memmap (avoids large RAM allocation)
        out_tag = f'{args.tag}_{dt_tag}'
        WRITE_CHUNK = 500
        n_train, n_test = len(train_idx), len(test_idx)

        log(f"  Writing cache_i_train_{out_tag}.npy  ({n_train} × {N_BINS}, chunked)...")
        i_train = np.lib.format.open_memmap(
            f'cache_i_train_{out_tag}.npy', mode='w+',
            dtype=np.float32, shape=(n_train, N_BINS))
        for s in range(0, n_train, WRITE_CHUNK):
            e = min(s + WRITE_CHUNK, n_train)
            i_train[s:e] = i_all[train_idx[s:e]].astype(np.float32)
            if s % (WRITE_CHUNK * 200) == 0 and s > 0:
                log(f"    {s}/{n_train}")
        log(f"  i_train written: {i_train.shape}")

        log(f"  Writing cache_i_test_{out_tag}.npy   ({n_test} × {N_BINS}, chunked)...")
        i_test = np.lib.format.open_memmap(
            f'cache_i_test_{out_tag}.npy', mode='w+',
            dtype=np.float32, shape=(n_test, N_BINS))
        for s in range(0, n_test, WRITE_CHUNK):
            e = min(s + WRITE_CHUNK, n_test)
            i_test[s:e] = i_all[test_idx[s:e]].astype(np.float32)
        log(f"  i_test written: {i_test.shape}")
        del i_all

        n_workers = min(args.workers, n_train)
        ...
        X_test = build_features_for_split(
            ...
            min(n_workers, n_test), args.chunk_size, 'test')

        log(f"  Saving cache_{{X,d}}_{{train,test}}_{out_tag}.npy ...")
