"""
stage3_v4_2.py — Stage 3 event detection pipeline, version 4.2
═════════════════════════════════════════════════════════════

Changes vs stage3_v4_1:

1. pearson_fn callback: optional callable injected by the runner.
   Signature:  pearson_fn(tp, factor_d_dict) -> (best_D_hat, best_factor, best_score)
   where factor_d_dict = {factor: D_hat_f} for all BL_FACTORS at the current step.

   When provided, the expansion loop uses best_D_hat (Pearson-selected) instead
   of the fixed factor=1.0 D_hat for:
       - tau_max lookup (_lookup_tau_max)
       - D_FAST_CUTOFF stopping check

   When None, behaviour is identical to stage3_v4_1 (full backward compatibility).

2. Event dict gains:
       pearson_best_factor : float — the winning BL_FACTOR at the final expansion step
                             (1.0 if pearson_fn was not provided)

All other logic is unchanged from v4.1.
"""
