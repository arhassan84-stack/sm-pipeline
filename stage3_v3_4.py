"""
stage3_v3_4.py — Stage 3 event detection pipeline, version 3.4
═══════════════════════════════════════════════════════════
...
def _step1(bundle: TraceBundle,
    ...
    for ev in cat1_raw:
        ...
        # Pre-check: if any non-excluded Cat-2 event with strictly higher
        # amplitude has its tp within this Cat-1 seed window, yield territory
        # to that Cat-2 event.  Without this check, the Cat-1 event would mark
        # the region as claimed — blocking the Cat-2 event's expansion —
        # even though the Cat-2 peak is the dominant signal in this area.
        dominated = any(
            o['id'] not in excluded
            and cl_s <= o['tp'] <= cl_e
            and o['h_peak'] > ev['h_peak']
            for o in cat2_raw
        )
        if dominated:
            excluded.add(ev['id'])
        ...
        ev['t_start'] = cl_s
        ev['t_end']   = cl_e
        # Exclude all peaks whose tp falls inside this window, but only if they
        # have not already been processed (i.e. do not yet own territory).
        # A peak that was processed earlier had higher amplitude and earned its
        # territory; it must not be retroactively removed by a broader but
        # lower-amplitude event processed later.
        for other in cat1_raw + cat2_raw:
            if other['id'] != ev['id'] and cl_s <= other['tp'] <= cl_e:
                # Only exclude if: (a) not yet processed, AND (b) this event
                # has equal or higher amplitude — never let a lower-amplitude
                # event claim territory that belongs to a higher-amplitude one.
                if 't_start' not in other and other['h_peak'] <= ev['h_peak']:
    ...
    for ev in cat2:
        ...
        for i in cat2_id_arr[mask]:
            ...
            if 't_start' not in other_c2 and other_c2['h_peak'] <= ev['h_peak']:
                excluded.add(int(i))
