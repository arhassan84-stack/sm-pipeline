"""
Multi-dt WaveNet — Option G: weighted loss (2× for d>=1).

Same architecture as multidt_B (fast version), with one change:
  - MSELoss replaced by weighted MSE: samples with d>=1 (log_d>=0) get weight 2.0,
    d<1 samples get weight 1.0. This directly up-pressures the model to improve
    the harder fast-diffuser regime without changing any other hyperparameter.

Label: wavenet_multidt_G
"""
...
LABEL        = 'wavenet_multidt_G'
D_GE1_WEIGHT = 2.0   # up-weight d>=1 samples (log_d >= 0) in MSE loss
CHANNELS     = 256
BATCH_SIZE   = 128
DL_WORKERS   = 6
MAX_EPOCHS   = 200
...
def weighted_mse(pred, target):
    """MSE with 2× weight on d>=1 samples (log_d >= 0)."""
    w = torch.where(target >= 0.0,
                    torch.full_like(target, D_GE1_WEIGHT),
...
for epoch in range(1, MAX_EPOCHS + 1):
    ...
    for t010b, t050b, t100b, xb, yb in loader:
        ...
        opt.zero_grad()
        with autocast():
            loss = weighted_mse(model(t010b, t050b, t100b, xb), yb)
...
summary = (
    ...
    f"  Loss: weighted MSE (d>=1 weight={D_GE1_WEIGHT}x) | Speedups: AvgPool{POOL010} on dt010 | CUDA streams | AMP\n"
    f"Params: {n_params:,}\n"
