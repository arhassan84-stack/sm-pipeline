"""
Multi-dt WaveNet — Option E: 4 independent branches (dt=0.05, 0.1, 0.5, 1.0ms).

Adds a dt=0.05ms branch (81920 bins) to the 3-branch multidt_B architecture,
giving finer temporal resolution for fast diffusers (d>=1) whose ACF decays
on sub-0.1ms timescales.

Architecture:
  dt=0.05ms (81920) → AvgPool(32) → 2560 → WaveNetBackbone(256) → 256-dim  [stream s005]
  dt=0.10ms (40960) → AvgPool(16) → 2560 → WaveNetBackbone(256) → 256-dim  [stream s010]
  dt=0.50ms  (8192) →   native   →      → WaveNetBackbone(256) → 256-dim  [stream s050]
  dt=1.00ms  (4096) →   native   →      → WaveNetBackbone(256) → 256-dim  [main stream]
  1208-dim features (4×302)     → feat_branch(1208→256→128)   → 128-dim  [main stream]
                                                                           ↓
  concat [4×256 + 128 = 1152] → Linear(1152,512) → ReLU → Linear(512,128) → ReLU → Linear(128,1)

Speedups retained from multidt_B:
  1. AvgPool(32) on dt005, AvgPool(16) on dt010: both → 2560 bins
  2. CUDA streams: branch005 and branch010 run concurrently on side streams
  3. AMP (FP16)
  4. BATCH_SIZE=128

Label: wavenet_multidt_E
"""
...
AUG_SHIFT    = 128
EVAL_BATCH   = 128
POOL005      = 32   # AvgPool factor for dt005 branch: 81920 → 2560
...
class AugFCSDatasetMultiDT4(Dataset):
    def __init__(self, i005_mmap, i005_idx, i010_mmap, i010_idx,
                 i050, i100, X_tensor, y_tensor):
        self.i005_mmap = i005_mmap; self.i005_idx = i005_idx
    ...
    def __getitem__(self, idx):
        t005 = np.array(self.i005_mmap[self.i005_idx[idx]], dtype=np.float32)
        ...
        a005 = aug(t005, s100 * 20)
        a010 = aug(t010, s100 * 10)
        a050 = aug(t050, s100 *  2)
        a100 = aug(t100, s100 *  1)
        return (torch.from_numpy(a005.astype(np.float32)),
                torch.from_numpy(a010.astype(np.float32)),
...
class WaveNetMultiDT_E(nn.Module):
    """4 independent WaveNet backbones with CUDA-stream parallelism."""
    def __init__(self, feat_dim, channels=256, dilations=None):
        super().__init__()
        self.branch005 = WaveNetBackbone(channels, dilations, avgpool=POOL005)  # 81920→2560
        ...
        # 4 branches × 256 + 128 features = 1152
...
t_total = time.time()
log(f"Device: {device}")
log(f"Speedups: AvgPool{POOL005} on dt005, AvgPool{POOL010} on dt010 | CUDA streams | AMP | BATCH={BATCH_SIZE}")
log("Loading multi-dt4 caches...")

i005_tr_mmap = np.load('cache_i_train_multidt4_dt005.npy', mmap_mode='r')
i010_tr_mmap = np.load('cache_i_train_multidt4_dt010.npy', mmap_mode='r')
i050_tr = np.load('cache_i_train_multidt4_dt050.npy').astype(np.float32)
i100_tr = np.load('cache_i_train_multidt4_dt100.npy').astype(np.float32)
X005_tr = np.load('cache_X_train_multidt4_dt005.npy').astype(np.float32)
X010_tr = np.load('cache_X_train_multidt4_dt010.npy').astype(np.float32)
X050_tr = np.load('cache_X_train_multidt4_dt050.npy').astype(np.float32)
X100_tr = np.load('cache_X_train_multidt4_dt100.npy').astype(np.float32)
d_train = np.load('cache_d_train_multidt4.npy')

i005_te_mmap = np.load('cache_i_test_multidt4_dt005.npy', mmap_mode='r')
i010_te_mmap = np.load('cache_i_test_multidt4_dt010.npy', mmap_mode='r')
i050_te = np.load('cache_i_test_multidt4_dt050.npy').astype(np.float32)
i100_te = np.load('cache_i_test_multidt4_dt100.npy').astype(np.float32)
X005_te = np.load('cache_X_test_multidt4_dt005.npy').astype(np.float32)
X010_te = np.load('cache_X_test_multidt4_dt010.npy').astype(np.float32)
X050_te = np.load('cache_X_test_multidt4_dt050.npy').astype(np.float32)
X100_te = np.load('cache_X_test_multidt4_dt100.npy').astype(np.float32)
d_test  = np.load('cache_d_test_multidt4.npy')

X_train = np.concatenate([X005_tr, X010_tr, X050_tr, X100_tr], axis=1)  # (N, 1208)
X_test  = np.concatenate([X005_te, X010_te, X050_te, X100_te], axis=1)
...
i005_val_n = norm_traces(np.array(i005_tr_mmap[tr_global[val_idx]], dtype=np.float32))
...
train_ds = AugFCSDatasetMultiDT4(
    i005_mmap=i005_tr_mmap, i005_idx=tr_global[tr_idx],
...
feat_dim = X_tr_sc.shape[1]  # 1208
model  = WaveNetMultiDT_E(feat_dim=feat_dim, channels=CHANNELS, dilations=DILATIONS).to(device)
...
log(f"Training {LABEL}  params={n_params:,}  (4 branches + CUDA streams + AMP)")
log(f"  dt005: AvgPool{POOL005}→{81920//POOL005} bins | dt010: AvgPool{POOL010}→{40960//POOL010} bins | dt050: 8192 | dt100: 4096")
log(f"  feat_dim={feat_dim} (4×302)  head_in={CHANNELS*4+128}")
...
for epoch in range(1, MAX_EPOCHS + 1):
    model.train(); ep = 0.0
    for t005b, t010b, t050b, t100b, xb, yb in loader:
        t005b = t005b.to(device, non_blocking=True)
        ...
        opt.zero_grad()
        with autocast():
            loss = crit(model(t005b, t010b, t050b, t100b, xb), yb)
    ...
    with torch.no_grad():
        vl = crit(
            batch_predict(lambda a, b, c, d, x: model(a, b, c, d, x),
                          torch.tensor(i005_val_n), torch.tensor(i010_val_n),
                          torch.tensor(i050_val_n), torch.tensor(i100_val_n),
                          X_val, bs=EVAL_BATCH),
            y_val).item()
...
def batch_predict_E(i005_mmap, i010_mmap, i_global, i050, i100, X_np, bs=128):
    n = len(i_global); outs = []
    ...
    with torch.no_grad(), autocast():
        for s in range(0, n, bs):
            sl = i_global[s:s+bs]
            chunk005 = np.array(i005_mmap[sl], dtype=np.float32)
            mu005  = chunk005.mean(axis=1, keepdims=True)
            sig005 = chunk005.std(axis=1,  keepdims=True) + 1e-8
            t005 = torch.tensor((chunk005 - mu005) / sig005).to(device)
            chunk010 = np.array(i010_mmap[sl], dtype=np.float32)
            mu010  = chunk010.mean(axis=1, keepdims=True)
            sig010 = chunk010.std(axis=1,  keepdims=True) + 1e-8
            t010 = torch.tensor((chunk010 - mu010) / sig010).to(device)
            outs.append(model(t005, t010,
...
logd_pred_te = batch_predict_E(i005_te_mmap, i010_te_mmap, te_global, i050_te, i100_te, X_te_sc).numpy()
log("Predicting train set...")
logd_pred_tr = batch_predict_E(i005_tr_mmap, i010_tr_mmap, tr_global, i050_tr, i100_tr, X_tr_sc).numpy()
...
summary = (
    f"Task: {LABEL}\n"
    f"Architecture: 4 independent WaveNet branches "
    f"(dt005 AvgPool{POOL005}→{81920//POOL005}bins, "
    f"dt010 AvgPool{POOL010}→{40960//POOL010}bins, "
    f"dt050 native, dt100) + 1208-dim features\n"
    f"  WaveNetMultiDT_E channels={CHANNELS} dilations={DILATIONS}\n"
    f"  Speedups: AvgPool{POOL005} on dt005 | AvgPool{POOL010} on dt010 | CUDA streams | AMP\n"
    f"Params: {n_params:,}\n"
