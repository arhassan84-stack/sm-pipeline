"""
CNN FusionNet with sample-weighted MSE loss.

Weight function (smooth, based on sqrt of d):
  w(d) = max(1.0, sqrt(d))

Effect:
  d=0.01 → w=1.0   d=0.1  → w=1.0
  d=1    → w=1.0   d=2    → w=1.41
  d=5    → w=2.24  d=10   → w=3.16

Validation loss uses UNWEIGHTED MSE (for fair early-stopping comparison).
Architecture and hyperparameters identical to model_cnn_90pct.
"""
...
# Sample weights: w = max(1, sqrt(d))
d_tr_vals = d_train[tr_idx]
w_arr = np.maximum(1.0, np.sqrt(d_tr_vals)).astype(np.float32)
log(f"Sample weights: min={w_arr.min():.2f}  max={w_arr.max():.2f}  mean={w_arr.mean():.2f}")
log(f"  d<1: mean_w={w_arr[d_tr_vals<1].mean():.2f}  d>=2: mean_w={w_arr[d_tr_vals>=2].mean():.2f}")

i_tr  = torch.tensor(i_train_n[tr_idx]);  X_tr = torch.tensor(X_tr_sc[tr_idx])
y_tr  = torch.tensor(log_d[tr_idx]);      w_tr = torch.tensor(w_arr)
...
loader = DataLoader(TensorDataset(i_tr, X_tr, y_tr, w_tr), batch_size=BATCH_SIZE, shuffle=True, pin_memory=True)
...
log(f"Training {LABEL}  params={n_params:,}  (weighted loss: w=sqrt(d))")
best_val, best_state, patience_count = float('inf'), None, 0

for epoch in range(1, MAX_EPOCHS+1):
    model.train(); ep = 0.0
    for ib, xb, yb, wb in loader:
        ib, xb, yb, wb = ib.to(device), xb.to(device), yb.to(device), wb.to(device)
        opt.zero_grad()
        pred = model(ib, xb)
    ...
    model.eval()
    with torch.no_grad():
        vl = val_crit(batch_predict(lambda i,x: model(i,x), i_val, X_val, bs=EVAL_BATCH), y_val).item()
    sched.step(epoch-1)
...
torch.save(model.state_dict(), 'model_cnn_wloss.pt')
np.save('scaler_cnn_wloss_mean.npy', feat_scaler.mean_)
np.save('scaler_cnn_wloss_scale.npy', feat_scaler.scale_)
log("Saved model_cnn_wloss.pt + scaler")
...
summary = (
    f"Task: {LABEL}\n"
    f"Architecture: FusionNet + weighted loss w=max(1,sqrt(d))\n"
    f"Params: {n_params:,}\n"
