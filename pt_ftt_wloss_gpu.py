"""
FT-Transformer with sample-weighted MSE loss.

Same weighting as CNN wloss: w(d) = max(1.0, sqrt(d))
  d<1 → w=1.0,  d=2 → w=1.41,  d=5 → w=2.24,  d=10 → w=3.16

Architecture identical to model_fttransformer (E=64, 4 layers, 8 heads).
Validation uses UNWEIGHTED MSE for fair early-stopping.
"""
...
w_tr  = torch.tensor(w_arr).to(device)
...
model = FTTransformer(n_features=X_train.shape[1], E=E, n_heads=N_HEADS,
                      n_layers=N_LAYERS, dim_ff=DIM_FF, dropout=DROPOUT).to(device)
...
log(f"  weighted loss w=max(1,sqrt(d))")
...
for epoch in range(1, MAX_EPOCHS+1):
    ...
    ep /= len(X_tr)
    model.eval()
    with torch.no_grad(): vl = val_crit(model(X_val), y_val).item()
    sched.step(epoch-1)
...
torch.save(model.state_dict(), 'model_ftt_wloss.pt')
np.save('scaler_ftt_wloss_mean.npy', scaler.mean_)
np.save('scaler_ftt_wloss_scale.npy', scaler.scale_)
log("Saved model_ftt_wloss.pt + scaler")
...
summary = (
    f"Task: {LABEL}\nArchitecture: FT-Transformer E={E} heads={N_HEADS} layers={N_LAYERS} + weighted loss\n"
    f"Params: {n_params:,}\n"
