"""
WaveNet wide + augmentation — trained on dt050 simulation data (dt=0.5ms, 8192 bins/trace).

Architecture identical to pt_wavenet_wide_aug_gpu.py (WaveNet1D channels=256).
AdaptiveAvgPool1d(1) makes the trace branch length-agnostic: 8192-pt input
is handled without any architectural changes.

Key differences vs the dt=1ms aug model:
  - Input traces: 8192 bins (dt=0.5ms, tMax=4096ms) instead of 4096
  - Augmentation shift: ±256 bins (proportional to trace length: 128/4096 * 8192)
  - Training data: cache_i_train_dt050.npy / cache_X_train_dt050.npy / cache_d_train_dt050.npy
  - Test data:     cache_i_test_dt050.npy  / cache_X_test_dt050.npy  / cache_d_test_dt050.npy
  - Augmentation applied online per batch (no pre-built aug cache needed)
  - Feature dim: 302 (same pipeline, parameterised by N=8192 in build_cache_dt.py)
...
# Augmentation parameters (scaled proportionally to 8192-bin traces)
AUG_SCALE_LO = 0.85
AUG_SCALE_HI = 1.15
AUG_NOISE_SD = 0.03
AUG_SHIFT    = 256     # ±256 bins  (same proportion as ±128/4096 on dt=1ms)
...
def augment_traces(traces, rng):
    ...
    Online trace augmentation (identical types to wavenet_wide_aug pre-built cache):
      ...
      3. Cyclic shift        ~ Uniform[-256, +256] bins
...
t_total = time.time()
log(f"Device: {device}")
log("Loading dt050 cache (8192 bins/trace)...")

i_train_raw = np.load('cache_i_train_dt050.npy').astype(np.float32)
X_train     = np.load('cache_X_train_dt050.npy').astype(np.float32)
d_train     = np.load('cache_d_train_dt050.npy')

i_test_raw  = np.load('cache_i_test_dt050.npy').astype(np.float32)
X_test      = np.load('cache_X_test_dt050.npy').astype(np.float32)
d_test      = np.load('cache_d_test_dt050.npy')
...
summary = (
    f"Task: {LABEL}\n"
    f"dt=0.5ms  N_BINS={N_BINS}  tMax=4096ms\n"
