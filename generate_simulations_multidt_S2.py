"""
Multi-dt FCS simulation generator — S2 / mCherry2 / 561 nm excitation.

Identical to generate_simulations_multidt.py except:
  EX_LAMBDA = 561.0 nm  (was 488.0 nm for GFP/ChS1)
  WXY       = 1e-3 * 0.51 * 561.0 / 1.2 = 0.238425 µm  (was 0.2074 µm)
  default --tag = multidt_S2
...
# ── Fixed physical parameters — S2 / mCherry2 / 561 nm ───────────────────────
EX_LAMBDA = 561.0                            # 561 nm (mCherry2) vs 488 nm (GFP)
NA        = 1.2
WXY       = 1e-3 * 0.51 * EX_LAMBDA / NA   # 0.238425 µm  (was 0.2074 µm)
D0_MIN    = 0.01
D0_MAX    = 14.12537544622754
NUM_D     = 1000
MAX_RATE  = 50000.0                          # counts/s  (unchanged)
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate multi-dt FCS traces for mCherry2/S2 (561 nm excitation)')
    parser.add_argument('--dts', type=float, nargs='+',
                        default=[0.1, 0.5, 1.0],
                        help='dt values in ms (default: 0.1 0.5 1.0)')
    ...
    parser.add_argument('--tag', type=str, default='multidt_S2',
                        help='Output filename prefix (default: multidt_S2)')
    ...
    log("=" * 60)
    log(f"Multi-dt FCS Simulation (mCherry2/S2)  tag={args.tag}")
    log(f"  EX_LAMBDA={EX_LAMBDA} nm  WXY={WXY*1e3:.4f} nm  NA={NA}")
    log("=" * 60)
    ...
    n_D_per_chunk = min(n_D_per_chunk, NUM_D // args.workers)
