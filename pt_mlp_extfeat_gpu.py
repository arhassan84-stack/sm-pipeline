"""
Improvement: MLP+BN+Dropout on 252 extended features
(207 original + 29 extra ACF lags + 16 segment variances).
Uses 50k split with extended feature cache.
"""
...
t_total = time.time()
log(f"Device: {device}")
log("Loading 50k extended feature cache (252 features)...")
X_train = np.load('cache_X_train_extfeat.npy'); d_train = np.load('cache_d_train.npy')
X_test  = np.load('cache_X_test_extfeat.npy');  d_test  = np.load('cache_d_test.npy')
...
log(f"Training {LABEL}  params={n_params:,}  in_dim={X_tr.shape[1]}...")
...
summary = (f"Task: {LABEL}\nArchitecture: 2048-1024-512-256-128 + BN + Dropout({DROPOUT})  Features: {X_train.shape[1]}\n"
...
ax=axes[0]; ax.scatter(d_test,d_pred_te,alpha=0.2,s=5,color='navy')
