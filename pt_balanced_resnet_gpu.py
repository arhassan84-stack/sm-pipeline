"""
Balanced training: ResNet-MLP with CosineAnnealingWarmRestarts.
...
idx_slow = np.where(d_train < 1.0)[0]
idx_fast = np.where(d_train >= 1.0)[0]
n_fast = len(idx_fast)
...
torch.save(model.state_dict(), 'model_balanced_resnet.pt')
np.save('scaler_balanced_resnet_mean.npy', scaler.mean_)
np.save('scaler_balanced_resnet_scale.npy', scaler.scale_)
log("Saved model_balanced_resnet.pt + scaler")
