"""
pt_wavenet_multidt_otf_gpu.py — On-the-fly multi-dt WaveNet (D prediction).

Simulates FCS traces on GPU each batch (2D Brownian motion, n=0 background),
computes the full feature pipeline via features_gpu.py, and trains
WaveNetMultiDT_B — the same architecture as multidt_B_fast.

No cache files are read during training; everything is generated on GPU.
GPU memory requirement: ~2-3 GB  (no large caches → only 32G SLURM RAM needed).

Design
------
  STEPS_PER_EPOCH : gradient steps per "epoch" (epoch is a scheduling concept,
                    not a dataset sweep — data is infinite)
  Warm-up phase   : WARMUP_BATCHES batches → fit StandardScaler on GPU features
  Validation set  : VAL_SIZE traces simulated once at startup, reused each epoch

Label: wavenet_multidt_otf
"""
...
CHANNELS         = 256
BATCH_SIZE       = 128
STEPS_PER_EPOCH  = 2000          # gradient steps per epoch
MAX_EPOCHS       = 200
PATIENCE         = 25
LR               = 5e-4
WEIGHT_DECAY     = 1e-4
...
POOL010          = 16
EVAL_BATCH       = 256
WARMUP_BATCHES   = 500           # batches to fit feature scaler
VAL_SIZE         = 10_000        # validation traces (fixed, generated once)
S2_CHUNK         = 30            # S2 pairs per IFFT (memory vs. speed)

# ── FCS physical constants (from generate_simulations_multidt.py) ─────────────
EX_LAMBDA = 488.0;  NA = 1.2
WXY       = 1e-3 * 0.51 * EX_LAMBDA / NA   # beam waist = 0.2074 µm
MAX_RATE  = 50_000.0                         # peak count rate (counts/s)
D0_MIN    = 0.01                             # µm²/s  (log-uniform sampling)
...
def simulate_batch(B: int) -> tuple:
    ...
    """
    # Log-uniform D sampling  [D0_MIN, D0_MAX]
    D = torch.empty(B, device=device).uniform_(
        math.log(D0_MIN), math.log(D0_MAX)).exp()

    # 2D Brownian motion at dt010 = 0.1 ms
    step_std = (2.0 * D * DT010_S).sqrt()                     # (B,)
    ...
    x = torch.cat([zeros, sx.cumsum(1)], dim=1)               # (B, N010)
    y = torch.cat([zeros, sy.cumsum(1)], dim=1)

    # Centre at midpoint  (matches generate_simulations_multidt.py convention)
    mid = N010 // 2 - 1
    x = x - x[:, mid:mid + 1]
    y = y - y[:, mid:mid + 1]

    # 2D Gaussian PSF → FCS intensity with Poisson shot noise
    em       = torch.exp(-2.0 * ((x / WXY) ** 2 + (y / WXY) ** 2))
    counts   = torch.poisson(DT010_S * MAX_RATE * em)         # (B, N010)
    I010     = counts / DT010_S                                # counts/s

    # Subsample to coarser dt by averaging (equivalent to summing counts / dt)
...
def norm_traces(t: torch.Tensor) -> torch.Tensor:
    """Zero-mean unit-variance normalisation along the time axis."""
    mu  = t.mean(1, keepdim=True)
    sig = t.std(1,  keepdim=True) + 1e-8
    return (t - mu) / sig


# ── Model (identical to WaveNetMultiDT_B / multidt_B_fast) ───────────────────
...
class WaveNetMultiDT_B(nn.Module):
    def __init__(self, feat_dim, ch=256):
        super().__init__()
...
# ── Feature constants (precomputed once per trace length) ─────────────────────
...
log(f"Warm-up: {WARMUP_BATCHES} batches × {BATCH_SIZE} = "
...
log(f"Generating fixed validation set ({VAL_SIZE} traces)...")
t_vs  = time.time()
...
log(f"  Validation set ready ({time.time() - t_vs:.1f}s)")
...
log(f"  AvgPool{POOL010} on dt010 branch | CUDA streams | AMP")
...
for epoch in range(1, MAX_EPOCHS + 1):
    ...
    for step in range(STEPS_PER_EPOCH):
        # ── Simulate batch on GPU ──
        I010, I050, I100, D = simulate_batch(BATCH_SIZE)

        # ── Compute features on GPU (no grad) ──
        with torch.no_grad():
            X010 = compute_features_gpu(I010, C010, S2_CHUNK)
            X050 = compute_features_gpu(I050, C050, S2_CHUNK)
            X100 = compute_features_gpu(I100, C100, S2_CHUNK)
...
summary = (
    ...
    f"  d <1  ({ms.sum():5d}): R²={r2_score(D_val[ms], D_pred[ms]):.4f}  "
    f"MAPE={mape(D_val[ms], D_pred[ms]):.1f}%\n"
    f"  d>=1  ({mf.sum():5d}): R²={r2_score(D_val[mf], D_pred[mf]):.4f}  "
    f"MAPE={mape(D_val[mf], D_pred[mf]):.1f}%\n"
