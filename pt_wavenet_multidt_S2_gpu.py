"""
Multi-dt WaveNet — S2/mCherry2 model (561 nm PSF).

Architecture identical to wavenet_multidt_B: 3 independent WaveNet branches
(dt010 with AvgPool16, dt050 native, dt100 native) + CUDA streams + AMP.

Trained on mCherry2/S2 simulations generated with EX_LAMBDA=561nm
(WXY=0.238425 µm vs 0.2074 µm for GFP/ChS1).

Label: wavenet_multidt_S2
"""
...
class WaveNetMultiDT_S2(nn.Module):
    """3 independent WaveNet backbones with CUDA-stream parallelism — S2/mCherry2."""
...
log("Loading multi-dt S2/mCherry2 caches...")

i010_tr_mmap = np.load('cache_i_train_multidt_S2_dt010.npy', mmap_mode='r')
i050_tr = np.load('cache_i_train_multidt_S2_dt050.npy').astype(np.float32)
i100_tr = np.load('cache_i_train_multidt_S2_dt100.npy').astype(np.float32)
X010_tr = np.load('cache_X_train_multidt_S2_dt010.npy').astype(np.float32)
X050_tr = np.load('cache_X_train_multidt_S2_dt050.npy').astype(np.float32)
X100_tr = np.load('cache_X_train_multidt_S2_dt100.npy').astype(np.float32)
d_train = np.load('cache_d_train_multidt_S2.npy')

i010_te_mmap = np.load('cache_i_test_multidt_S2_dt010.npy', mmap_mode='r')
i050_te = np.load('cache_i_test_multidt_S2_dt050.npy').astype(np.float32)
i100_te = np.load('cache_i_test_multidt_S2_dt100.npy').astype(np.float32)
X010_te = np.load('cache_X_test_multidt_S2_dt010.npy').astype(np.float32)
X050_te = np.load('cache_X_test_multidt_S2_dt050.npy').astype(np.float32)
X100_te = np.load('cache_X_test_multidt_S2_dt100.npy').astype(np.float32)
d_test  = np.load('cache_d_test_multidt_S2.npy')
...
model  = WaveNetMultiDT_S2(feat_dim=feat_dim, channels=CHANNELS, dilations=DILATIONS).to(device)
...
def batch_predict_S2(i010_mmap, i010_idx, i050, i100, X_np, bs=128):
    n = len(i010_idx); outs = []
...
logd_pred_te = batch_predict_S2(i010_te_mmap, te_global, i050_te, i100_te, X_te_sc).numpy()
log("Predicting train set...")
logd_pred_tr = batch_predict_S2(i010_tr_mmap, tr_global, i050_tr, i100_tr, X_tr_sc).numpy()
...
summary = (
    ...
    f"  WaveNetMultiDT_S2 channels={CHANNELS} dilations={DILATIONS}\n"
    f"  Speedups: AvgPool{POOL010} on dt010 | CUDA streams | AMP\n"
    f"  PSF: EX_LAMBDA=561nm (mCherry2/S2)  WXY=0.2384µm\n"
    f"Params: {n_params:,}\n"
