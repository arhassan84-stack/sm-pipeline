"""
Multi-dt WaveNet — Option B: 3 independent branches.

Each dt value has its own WaveNet1D backbone (channels=256), processing traces
at their native resolution. AdaptiveAvgPool1d(1) gives a 256-dim embedding per
branch. All three embeddings (768-dim total) + features (906-dim) → shared head.
...
AUG_SCALE_HI = 1.15
AUG_NOISE_SD = 0.03
AUG_SHIFT    = 128      # bins in dt100 space; ×10 / ×2 for dt010/dt050
EVAL_BATCH   = 64
...
class WaveNetMultiDT_B(nn.Module):
    """3 independent WaveNet backbones, one per dt."""
...
log(f"Training {LABEL}  params={n_params:,}  (3 independent branches)")
...
summary = (
    f"Task: {LABEL}\n"
    f"Architecture: 3 independent WaveNet branches (dt010 native, dt050 native, dt100) + 906-dim features\n"
