"""
WaveNet seed=2 diversity variant — identical architecture to pt_wavenet,
different torch initialization seed.  Provides ensemble diversity via
independent weight initialization on the same receptive field.

Architecture: same as pt_wavenet (channels=128, dilations=[1,2,4,8,16,32,64,128])
Seed: torch.manual_seed(2) before model construction.
Saves pred_logd_wavenet_s2_train/test.npy for meta-learner.
"""
...
log(f"Training {LABEL}  seed={SEED}  params={n_params:,}")
...
torch.save(model.state_dict(), f'model_wavenet_s2.pt')
np.save('scaler_wavenet_s2_mean.npy', feat_scaler.mean_)
np.save('scaler_wavenet_s2_scale.npy', feat_scaler.scale_)
log("Saved model_wavenet_s2.pt + scaler")
...
np.save('pred_logd_wavenet_s2_train.npy', logd_pred_tr)
np.save('pred_logd_wavenet_s2_test.npy',  logd_pred_te)
log("Saved pred_logd_wavenet_s2_train/test.npy")
...
summary = (
    f"Task: {LABEL}\n"
    f"Architecture: WaveNet1D channels={CHANNELS} dilations={DILATIONS} seed={SEED}\n"
