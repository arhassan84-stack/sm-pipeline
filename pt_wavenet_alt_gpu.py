"""
WaveNet alternative dilation pattern for FCS diffusion prediction.

Motivation: The original WaveNet uses dilations [1,2,4,8,16,32,64,128] with RF=2040 original lags,
covering ~half the 4096-lag FCS trace.  This variant extends to 10 blocks:
  dilations = [1, 2, 4, 8, 16, 32, 64, 128, 256, 512]
  RF = 2*(1+2+4+8+16+32+64+128+256+512) * 4 = 8184 original lags
This covers the FULL FCS trace (4096 lags) with 2x margin, ensuring the complete
slow-diffusion ACF decay is captured even for the smallest d values (d~0.1).

Architecture:
  - Same as pt_wavenet except 10 dilated res-blocks instead of 8
  - channels=128, stride-4 input projection, GAP + feature branch + fusion head
  - Additional 2 blocks add ~54k params (total ~634k)

Saves pred_logd_wavenet_alt_train/test.npy for meta-learner.
"""
...
WEIGHT_DECAY = 1e-4
T0           = 30
DILATIONS    = [1, 2, 4, 8, 16, 32, 64, 128, 256, 512]   # 10 blocks, RF=8184 original lags
...
log(f"  Receptive field: {rf_reduced} (reduced) = {rf_original} (original) lags  [covers full 4096-lag trace]")
...
torch.save(model.state_dict(), 'model_wavenet_alt.pt')
np.save('scaler_wavenet_alt_mean.npy', feat_scaler.mean_)
np.save('scaler_wavenet_alt_scale.npy', feat_scaler.scale_)
log("Saved model_wavenet_alt.pt + scaler")
...
np.save('pred_logd_wavenet_alt_train.npy', logd_pred_tr)
np.save('pred_logd_wavenet_alt_test.npy',  logd_pred_te)
log("Saved pred_logd_wavenet_alt_train/test.npy")
...
summary = (
    ...
    f"  Receptive field: {rf_original} original lags (covers full 4096-lag FCS trace)\n"
    f"Params: {n_params:,}\n"
