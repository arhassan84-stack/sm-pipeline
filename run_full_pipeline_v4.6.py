...
Changes vs v4.5:
  - Combined matching stream: a new parallel replica-selection criterion
    that combines Pearson correlation and AUC similarity into a single
    generalizable [0, 1] score.  Three AUC components are implemented:
      A  AUC Ratio     min/max of nAUC values
      B  Sørensen-Dice 1 − |diff| / (sum)
      C  Log-Ratio     exp(−|log(nAUC_real / nAUC_sim)|)
    Two combination methods:
      II  Geometric mean   sqrt(pearson × auc_score)
      III Harmonic mean    2·p·a / (p + a)
    The geometric mean (II) with AUC-Ratio (A) is used as the primary
    combining score; its folder is geometric_opt/.
  - All six scores (pearson_comb, auc_ratio, auc_dice, auc_logratio,
    combined_geo, combined_harm) stored per (event, factor) row in
    metadata.csv.
  - geometric_opt/ subfolder: combined-best event plot (using
    combined-selected replica) saved with 'g_' prefix.
  - Per-measurement + grand-summary combined factor frequency bar charts
    saved to geometric_opt/.
  - Comparison histogram and scatter updated with a 4th series
    (combined stream, darkorchid).
...
PIPELINE_VERSION = 'v4.6'   # hardcoded — do not import from version.py
...
AUC_OPT_DIR      = EVT_DIR / 'auc_opt'
COMBINED_OPT_DIR = EVT_DIR / 'geometric_opt'
for d in [OUT_DIR, SIM_DIR, EVT_DIR, OPT_DIR, PHOT_DIR,
          KNEE_OPT_DIR, AUC_OPT_DIR, COMBINED_OPT_DIR]:
...
def make_comparison_d_histogram(stem, ch_id, d_pearson, d_knee, d_auc, d_combined,
                                out_path):
    """Overlapping D̂ histograms: Pearson-opt / Knee-opt / AUC-opt / Combined-opt."""
    d_p = np.array([v for v in d_pearson  if np.isfinite(v) and v > 0])
    d_k = np.array([v for v in d_knee     if np.isfinite(v) and v > 0])
    d_a = np.array([v for v in d_auc      if np.isfinite(v) and v > 0])
    d_c = np.array([v for v in d_combined if np.isfinite(v) and v > 0])
    if len(d_p) == 0 and len(d_k) == 0 and len(d_a) == 0 and len(d_c) == 0:
        return
    ...
    if len(d_p) > 0:
        ax.hist(d_p, bins=bins, alpha=0.45, color='steelblue',
    ...
    if len(d_k) > 0:
        ax.hist(d_k, bins=bins, alpha=0.45, color='orange',
                edgecolor='white', linewidth=0.4,
    ...
    if len(d_a) > 0:
        ax.hist(d_a, bins=bins, alpha=0.45, color='mediumseagreen',
                edgecolor='white', linewidth=0.4,
                label=f'AUC opt      (n={len(d_a)})')
        med_a = float(np.median(d_a))
        ax.axvline(med_a, color='mediumseagreen', lw=1.5, ls='--',
                   label=f'median = {med_a:.3f} µm²/s')

    if len(d_c) > 0:
        ax.hist(d_c, bins=bins, alpha=0.45, color='darkorchid',
                edgecolor='white', linewidth=0.4,
                label=f'Combined opt (n={len(d_c)})')
        med_c = float(np.median(d_c))
        ax.axvline(med_c, color='darkorchid', lw=1.5, ls='--',
                   label=f'median = {med_c:.3f} µm²/s')
    ...
    ax.set_xlabel('D̂  (µm²/s)', fontsize=FS_LABEL, color='black')
    ax.set_ylabel('Events',      fontsize=FS_LABEL, color='black')
    ax.set_title(f'{stem} / {ch_id}  —  Pearson vs Knee vs AUC vs Combined D̂',
                 fontsize=FS_TITLE, color='black')
...
def make_scatter_window_d(stem, ch_id,
                          ...
                          ws_knee,    d_knee,
                          ws_auc,     d_auc,
                          ws_combined, d_combined,
                          out_path):
    """Scatter: window size vs D̂ — Pearson / Knee / AUC / Combined opt."""
    ...
    if len(ws_auc) > 0:
        ax.scatter(ws_auc, d_auc,
                   color='mediumseagreen', marker='s', s=30, alpha=0.7, linewidths=0,
                   label=f'AUC opt      (n={len(ws_auc)})')
    if len(ws_combined) > 0:
        ax.scatter(ws_combined, d_combined,
                   color='darkorchid', marker='D', s=30, alpha=0.7, linewidths=0,
                   label=f'Combined opt (n={len(ws_combined)})')
...
def make_event_metric_histograms(stem, ch_id, metrics, out_dir):
    """
    Four per-measurement histograms of single-molecule event quality metrics.
    Outliers are identified with the IQR × 1.5 rule and highlighted in red.

    Parameters
    ----------
    metrics : list of dict, one entry per event, keys:
        peak_height  — max intensity within event window (kHz)
        excess       — peak_height − edge_baseline (kHz)
        rel_excess   — excess / peak_height  (fraction 0–1)
        phot_ratio   — baseline-subtracted photons / raw photons (fraction 0–1)
    """
    if not metrics:
        return

    PLOTS = [
        ('peak_height', 'Peak height (kHz)',                        'steelblue'),
        ('excess',      'Peak height above baseline (kHz)',         'darkorange'),
        ('rel_excess',  '(Peak \u2212 baseline) / Peak',           'mediumseagreen'),
        ('phot_ratio',  'Baseline-subtracted / raw photon count',   'mediumpurple'),
    ]

    def _outlier_bounds(vals):
        q1, q3 = np.percentile(vals, [25, 75])
        iqr    = q3 - q1
        return q1 - 1.5 * iqr, q3 + 1.5 * iqr

    n_ev = len(metrics)
    for key, xlabel, color in PLOTS:
        vals = np.array([m[key] for m in metrics], dtype=float)
        vals = vals[np.isfinite(vals)]
        if len(vals) == 0:
            continue

        lo, hi   = _outlier_bounds(vals)
        out_mask = (vals < lo) | (vals > hi)
        outliers = vals[out_mask]
        ...
        ax.hist(vals, bins=bins, color=color, alpha=0.60,
                edgecolor='white', linewidth=0.4,
                label=f'All  (n={len(vals)})')
        if n_out > 0:
            ax.hist(outliers, bins=bins, color='red', alpha=0.70,
                    edgecolor='white', linewidth=0.4,
                    label=f'Outliers  (n={n_out})')
            ax.axvline(lo, color='red', lw=1.2, ls='--', alpha=0.8)
            ax.axvline(hi, color='red', lw=1.2, ls='--', alpha=0.8)
        ...
        ax.legend(fontsize=FS_TICK)
        _spines(ax)
        ax.set_xlabel(xlabel,   fontsize=FS_LABEL, color='black')
        ax.set_ylabel('Events', fontsize=FS_LABEL, color='black')
        ax.set_title(f'{stem} / {ch_id}  \u2014  {xlabel}  (n={n_ev})',
                     fontsize=FS_TITLE, color='black')

        fname    = f'{stem}_{ch_id}_hist_{key}.png'
        fig.tight_layout()
        fig.savefig(out_dir / fname, dpi=150, facecolor='white', bbox_inches='tight')
        plt.close(fig)
        print(f'    hist_{key} \u2192 {fname}')
...
# ══════════════════════════════════════════════════════════════════════════════
# Generalizable matching criteria  (all scores ∈ [0, 1])
# ══════════════════════════════════════════════════════════════════════════════

def _score_auc_ratio(nauc_real, nauc_sim):
    """
    AUC Ratio  (Component A)
    ------------------------
    Name     : Normalised-AUC ratio
    Equation : S_A = min(nAUC_real, nAUC_sim) / max(nAUC_real, nAUC_sim)
    Range    : [0, 1]   (1 = perfect match, 0 = one value is zero)

    Measures how closely the peak-normalised area under the
    baseline-subtracted transit matches between real and simulated traces.
    The ratio form is symmetric and bounded in [0, 1] for any non-negative
    inputs; no parameter tuning is required.
    """
    lo = min(nauc_real, nauc_sim)
    hi = max(nauc_real, nauc_sim)
    return float(lo / hi) if hi > 0 else 0.0


def _score_auc_dice(nauc_real, nauc_sim):
    """
    AUC Sørensen-Dice  (Component B)
    ---------------------------------
    Name     : Sørensen-Dice coefficient on nAUC values
    Equation : S_B = 1 − |nAUC_real − nAUC_sim| / (nAUC_real + nAUC_sim)
                   = 2·min(nAUC_real, nAUC_sim) / (nAUC_real + nAUC_sim)
    Range    : [0, 1]   (1 = perfect match, → 0 as mismatch grows)

    The Sørensen-Dice coefficient penalises absolute difference relative
    to the sum, making it less sensitive to very small values than the
    ratio.  It is the generalisation of the F1 score to scalar quantities.
    """
    denom = nauc_real + nauc_sim
    if denom <= 0:
        return 0.0
    return float(1.0 - abs(nauc_real - nauc_sim) / denom)


def _score_auc_logratio(nauc_real, nauc_sim):
    """
    AUC Log-Ratio  (Component C)
    -----------------------------
    Name     : Exponential log-ratio similarity
    Equation : S_C = exp(−|log(nAUC_real / nAUC_sim)|)
    Range    : (0, 1]   (1 = perfect match, → 0 as log-ratio diverges)

    Compares nAUC values on the logarithmic scale, making it invariant to
    multiplicative distortions.  The exponential transform maps the
    absolute log-ratio ∈ [0, ∞) to a similarity in (0, 1].  Returns 0
    when either value is ≤ 0.
    """
    if nauc_real <= 0 or nauc_sim <= 0:
        return 0.0
    return float(np.exp(-abs(np.log(nauc_real / nauc_sim))))


def _score_geometric(scores):
    """
    Geometric Mean Combination  (Option II)
    ----------------------------------------
    Name     : Geometric mean of N individual [0, 1] criteria scores
    Equation : S_geo = (∏ᵢ sᵢ)^(1/N)
    Range    : [0, 1]

    Treats all criteria equally (no weights) and returns zero if ANY
    individual score is zero — making it strict: every criterion must be
    satisfied to some degree.  Generalises to any number of criteria
    without parameter tuning.  Sensitive to outlier-low scores.
    """
...
def _score_harmonic(scores):
    """
    Harmonic Mean Combination  (Option III)
    ----------------------------------------
    Name     : Harmonic mean of N individual [0, 1] criteria scores
    Equation : S_harm = N / (∑ᵢ 1/sᵢ)   for all sᵢ > 0
                      = 0                  if any sᵢ = 0
    Range    : [0, 1]

    The harmonic mean is the F-score generalised to N criteria.  Like the
    geometric mean it is parameter-free and symmetric, but less sensitive
    to near-zero scores (a score of 0.01 does not collapse the result to
    ~0 as strongly).  Still penalises any weak criterion.  Generalises to
    any number of criteria without parameter tuning.
    """
    scores = [max(0.0, s) for s in scores]
...
def _combined_best_match(t_peak, i100, edge_bl, factor, d_sim, i_sim,
                         unique_d, log_unique, D_hat):
    """
    Select the best-matching sim replica by geometric-mean combined score.

    For each candidate replica (all sims whose D matches D_hat):
      1. Pearson correlation between peak-normalised real and sim windows
      2. nAUC_real: peak-normalised integral of factor-subtracted real window
      3. nAUC_sim : peak-normalised integral of each sim replica
      4. AUC component scores A (ratio), B (Dice), C (log-ratio)
      5. Combined scores II (geometric), III (harmonic) of [pearson, auc_ratio]

    The primary combining score is the geometric mean (II) using AUC-Ratio
    (A) as the AUC component.

    Returns
    -------
    tuple (chosen_row_idx, D_sim_val,
           pearson, auc_ratio, auc_dice, auc_logratio,
           combined_geo, combined_harm)
    """
    ...
    # Real window (centred on t_peak ± EVT_HALF), zero-padded if needed
    ...
    # Real nAUC (factor-subtracted, peak-normalised)
    real_bl_seg = np.maximum(0.0, real_seg - factor * edge_bl)
    real_pk     = real_bl_seg.max()
    nauc_real   = float(real_bl_seg.sum() / real_pk) if real_pk > 0 else 0.0

    # Pearson: peak-normalise both real and each replica
    r_max     = real_seg.max()
    ...
    pearson_arr = num / denom    # shape (n_replicas,)

    # Per-replica nAUC (sim, peak-normalised)
    sim_pk_arr   = sims.max(axis=1)
    nauc_sim_arr = np.where(sim_pk_arr > 0,
                            sims.sum(axis=1) / sim_pk_arr,
                            0.0)

    # Per-replica combined scores
    n_rep             = len(same_d)
    auc_ratio_arr     = np.zeros(n_rep, dtype=np.float64)
    auc_dice_arr      = np.zeros(n_rep, dtype=np.float64)
    auc_logratio_arr  = np.zeros(n_rep, dtype=np.float64)
    combined_geo_arr  = np.zeros(n_rep, dtype=np.float64)
    combined_harm_arr = np.zeros(n_rep, dtype=np.float64)

    for k in range(n_rep):
        pr = float(pearson_arr[k])
        ns = float(nauc_sim_arr[k])
        ar = _score_auc_ratio(nauc_real, ns)
        ad = _score_auc_dice(nauc_real, ns)
        al = _score_auc_logratio(nauc_real, ns)
        auc_ratio_arr[k]     = ar
        auc_dice_arr[k]      = ad
        auc_logratio_arr[k]  = al
        # Primary AUC component for combination: AUC Ratio (A)
        combined_geo_arr[k]  = _score_geometric([max(0.0, pr), ar])
        combined_harm_arr[k] = _score_harmonic( [max(0.0, pr), ar])

    best_idx = int(np.argmax(combined_geo_arr))
    chosen   = int(same_d[best_idx])

    return (chosen, D_sim_val,
            float(pearson_arr[best_idx]),
            float(auc_ratio_arr[best_idx]),
            float(auc_dice_arr[best_idx]),
            float(auc_logratio_arr[best_idx]),
            float(combined_geo_arr[best_idx]),
            float(combined_harm_arr[best_idx]))
...
def make_event_plot_3panel(stem, ch_id, t_peak, i100, baseline,
                           ...
                           combined_geo_score=None, is_best_combined=False):
    ...
    if is_best_combined:
        sup_text  = f'\u2605 Best Combined  |  factor = {factor:.2f}'
        sup_color = 'darkorchid'
    elif is_best_pearson:
    ...
    if combined_geo_score is not None:
        geo_color  = 'darkorchid' if is_best_combined else '#888888'
        geo_weight = 'bold'       if is_best_combined else 'normal'
        ax_bot.text(0.97, 0.67, f'geo = {combined_geo_score:.3f}',
                    transform=ax_bot.transAxes,
                    fontsize=FS_ANNOT, ha='right', va='top',
                    color=geo_color, fontweight=geo_weight)
    if y_lim:
...
try:
    ...
    # v4.33/v4.6: grand-summary accumulators (ch_id → {factor: count})
    all_best_factor_counts     = {}
    all_knee_factor_counts     = {}
    all_auc_factor_counts      = {}
    all_combined_factor_counts = {}
    ...
    for stem in FILES:
        ...
        for ch_id in meas.channel_ids:
            ...
            # v4.33/v4.6: per-measurement frequency counters (reset each channel)
            best_factor_counts     = {f: 0 for f in BL_FACTORS}
            knee_factor_counts     = {f: 0 for f in BL_FACTORS}
            auc_factor_counts      = {f: 0 for f in BL_FACTORS}
            combined_factor_counts = {f: 0 for f in BL_FACTORS}

            # v4.35/v4.6: D_hat + window-size accumulators for comparison plots
            d_pearson_list   = []
            d_knee_list      = []
            d_auc_list       = []
            d_combined_list  = []
            ...
            # v4.5: per-event quality metrics for histograms
            event_metrics   = []
            ...
            for ev in events:
                ...
                # Event boundary + edge baseline (needed for AUC and combined scoring)
                ...
                # Pass 1a: Pearson best match per factor (post-hoc, for plotting)
                ...
                # Pass 1b: Combined best match per factor (selects replica by geo mean)
                combined_results = {}  # factor → (chosen, D_sim_val, pearson, auc_ratio,
                                       #            auc_dice, auc_logratio, geo, harm, sim_raw)
                for factor in BL_FACTORS:
                    D_hat_f = ev['d_hat_by_factor'].get(factor, np.nan)
                    if np.isnan(D_hat_f) or D_hat_f <= 0:
                        continue
                    res = _combined_best_match(
                        t_peak, i100, edge_bl, factor,
                        d_sim, i_sim, unique_d, log_unique, D_hat_f)
                    c_cho, c_ds, c_pr, c_ar, c_ad, c_al, c_geo, c_harm = res
                    c_sim_raw = i_sim[c_cho, WIN_S : WIN_E + 1].astype(np.float32)
                    combined_results[factor] = (c_cho, c_ds, c_pr, c_ar, c_ad,
                                                c_al, c_geo, c_harm, c_sim_raw)

                combined_best_factor = (
                    max(combined_results, key=lambda f: combined_results[f][6])
                    if combined_results else None)
                if combined_best_factor is not None:
                    combined_factor_counts[combined_best_factor] += 1

                # Photon counts for all 13 factors
                ...
                # v4.5: AUC best factor — minimise |nAUC_real − nAUC_sim|
                # Uses Pearson-selected replica per factor; baseline-subtracted real trace
                tl_cent = max(0, t_peak - EVT_HALF)
                tr_cent = min(N_tr - 1, t_peak + EVT_HALF)
                auc_scores = {}
                for _f, (_ch, _ds, _pr, _sr) in factor_results.items():
                    real_bl_seg = np.maximum(
                        0.0, i100[tl_cent : tr_cent + 1].astype(np.float32) - _f * edge_bl)
                    real_pk   = real_bl_seg.max()
                    real_nauc = float(real_bl_seg.sum() / real_pk) if real_pk > 0 else 0.0
                    sim_pk    = _sr.max()
                    sim_nauc  = float(_sr.sum() / sim_pk) if sim_pk > 0 else 0.0
                    auc_scores[_f] = -abs(real_nauc - sim_nauc)
                auc_best_factor = max(auc_scores, key=auc_scores.get)
                auc_factor_counts[auc_best_factor] += 1

                # v4.35/v4.6: accumulate D_hat + window size for comparison plots
                ...
                d_auc_list.append(ev['d_hat_by_factor'][auc_best_factor])
                ws_auc_list.append(win_ms)
                if combined_best_factor is not None:
                    d_combined_list.append(ev['d_hat_by_factor'].get(
                        combined_best_factor, np.nan))
                    ws_combined_list.append(win_ms)

                # v4.5: quality metrics for per-measurement histograms
                peak_height = float(i100[tl_eff : tr_eff + 1].max())
                excess      = peak_height - edge_bl
                rel_excess  = excess / peak_height if peak_height > 0 else 0.0
                phot_ratio  = (phot_by_factor[best_factor] / raw_photon_count
                               if raw_photon_count > 0 else 0.0)
                event_metrics.append({
                    'peak_height': peak_height,
                    'excess':      excess,
                    'rel_excess':  rel_excess,
                    'phot_ratio':  phot_ratio,
                })

                # Pass 2: save Pearson-stream sim traces + event plots (all 13 factors)
                for factor, (chosen, D_sim_val, pearson, sim_raw) in factor_results.items():
                    is_best  = (factor == best_factor)
                    is_knee  = (factor == knee_factor)
                    is_auc   = (factor == auc_best_factor)
                    ...
                    if is_auc:
                        shutil.copy2(out_path, AUC_OPT_DIR  / f'g_{out_path.name}')

                    # v4.6: combined scores for this factor (from combined-selected replica)
                    comb = combined_results.get(factor)
                    if comb:
                        _, _, c_pr, c_ar, c_ad, c_al, c_geo, c_harm, _ = comb
                    else:
                        c_pr = c_ar = c_ad = c_al = c_geo = c_harm = np.nan
                    ...
                    all_meta_rows.append({
                        ...
                        'auc_best_factor':           auc_best_factor,
                        # v4.6: combined-stream scores (from combined-selected replica)
                        'pearson_comb':              c_pr,
                        'auc_ratio':                 c_ar,
                        'auc_dice':                  c_ad,
                        'auc_logratio':              c_al,
                        'combined_geo':              c_geo,
                        'combined_harm':             c_harm,
                        'combined_best_factor': (combined_best_factor
                                                 if combined_best_factor is not None
                                                 else np.nan),
                    })
                ...
                # Pass 3: combined-stream event plot (combined-selected replica,
                #          combined_best_factor only → saved directly to geometric_opt/)
                if combined_best_factor is not None and combined_best_factor in combined_results:
                    c_cho, c_ds, c_pr, c_ar, c_ad, c_al, c_geo, c_harm, c_sim_raw = \
                        combined_results[combined_best_factor]
                    c_sim_khz = c_sim_raw * 1e-3
                    fac_tag   = f'f{int(round(combined_best_factor * 100)):03d}'
                    comb_path = (COMBINED_OPT_DIR /
                                 f'g_{stem}_{ch_id}_event_{t_peak:07d}ms_{fac_tag}.png')
                    make_event_plot_3panel(
                        stem, ch_id, t_peak, i100, baseline,
                        ev['t_left'], ev['t_right'],
                        ev['d_hat_by_factor'][combined_best_factor],
                        c_sim_khz, c_ds, comb_path,
                        category=ev.get('category'),
                        factor=combined_best_factor,
                        pearson_score=c_pr,
                        is_best_pearson=False,
                        combined_geo_score=c_geo,
            ...
            # v4.5: per-measurement event quality metric histograms
            make_event_metric_histograms(stem, ch_id, event_metrics, OUT_DIR)

            # v4.5/v4.6: comparison plots (Pearson / Knee / AUC / Combined)
            if n_plotted > 0:
                make_comparison_d_histogram(
                    stem, ch_id,
                    d_pearson_list, d_knee_list, d_auc_list, d_combined_list,
                    OUT_DIR / f'{stem}_{ch_id}_d_hist_compare.png')
                make_scatter_window_d(
                    stem, ch_id,
                    ws_pearson_list, d_pearson_list,
                    ws_knee_list,    d_knee_list,
                    ws_auc_list,     d_auc_list,
                    ws_combined_list, d_combined_list,
                    OUT_DIR / f'{stem}_{ch_id}_scatter_window_d.png')

            # v4.33/v4.5/v4.6: per-measurement frequency bar charts
            if n_plotted > 0:
                ...
                make_factor_freq_plot(
                    f'{stem} / {ch_id}  —  AUC best factor frequency',
                    auc_factor_counts,
                    AUC_OPT_DIR      / f'{stem}_{ch_id}_auc_factor_freq.png')
                make_factor_freq_plot(
                    f'{stem} / {ch_id}  —  Combined (geo) best factor frequency',
                    combined_factor_counts,
                    COMBINED_OPT_DIR / f'{stem}_{ch_id}_combined_factor_freq.png')
                ...
                if ch_id not in all_auc_factor_counts:
                    all_auc_factor_counts[ch_id]      = {f: 0 for f in BL_FACTORS}
                if ch_id not in all_combined_factor_counts:
                    all_combined_factor_counts[ch_id] = {f: 0 for f in BL_FACTORS}
                for f in BL_FACTORS:
                    all_best_factor_counts[ch_id][f]     += best_factor_counts[f]
                    all_knee_factor_counts[ch_id][f]     += knee_factor_counts[f]
                    all_auc_factor_counts[ch_id][f]      += auc_factor_counts[f]
                    all_combined_factor_counts[ch_id][f] += combined_factor_counts[f]

            print(f'  [{ch_id}]  {n_plotted} event plots saved  → event_plots/')

    # v4.33/v4.5/v4.6: grand-summary frequency bar charts (one per channel)
    ...
    for ch_id, counts in all_auc_factor_counts.items():
        make_factor_freq_plot(
            f'All measurements / {ch_id}  —  AUC best factor frequency',
            counts,
            AUC_OPT_DIR      / f'all_{ch_id}_auc_factor_freq.png')
    for ch_id, counts in all_combined_factor_counts.items():
        make_factor_freq_plot(
            f'All measurements / {ch_id}  —  Combined (geo) best factor frequency',
            counts,
            COMBINED_OPT_DIR / f'all_{ch_id}_combined_factor_freq.png')
    ...
    with open(meta_path, 'w', newline='') as f:
        writer = csv.DictWriter(
            f, fieldnames=['stem', 'ch_id', 't_peak_ms', 'factor',
                           ...
                           'knee_factor', 'auc_best_factor',
                           # v4.6: combined-stream scores
                           'pearson_comb', 'auc_ratio', 'auc_dice', 'auc_logratio',
                           'combined_geo', 'combined_harm', 'combined_best_factor'])
        writer.writeheader()
