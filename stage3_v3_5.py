"""
stage3_v3_5.py — Stage 3 event detection pipeline, version 3.5
═══════════════════════════════════════════════════════════
...
def _step1(bundle: TraceBundle,
    ...
    for ev in cat1_raw:
        ...
        ev['t_start'] = cl_s
        ev['t_end']   = cl_e
        # Exclude Cat-1 peaks whose tp falls inside this window, but only if
        # they have not already been processed (i.e. do not yet own territory).
    ...
    for ev, D_hat in zip(seed_events, initial_d_hats):
        ...
        if not fast_group:
            ...
            while w < W_MAX_MS:
                ...
                # Stop 1: W_MAX reached OR new territory already claimed by
                # another event of the SAME category.  Territory of the
                # opposite category is transparent — Cat-1 expands through
                # Cat-2 windows and vice versa.
                if (w_new >= W_MAX_MS
