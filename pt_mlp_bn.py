...
model = MLP_BN(in_dim=X_tr.shape[1], dropout=DROPOUT)
optimizer = torch.optim.AdamW(model.parameters(), lr=LR, weight_decay=WEIGHT_DECAY)
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
    optimizer, mode='min', factor=0.5, patience=10, verbose=False)
criterion = nn.MSELoss()

log(f"Training {LABEL}  params={sum(p.numel() for p in model.parameters()):,}...")
...
for epoch in range(1, MAX_EPOCHS + 1):
    ...
    if epoch % 10 == 0:
        log(f"Epoch {epoch:3d}  train_loss={ep_loss:.5f}  val_loss={val_loss:.5f}  lr={optimizer.param_groups[0]['lr']:.2e}  patience={patience_count}")
