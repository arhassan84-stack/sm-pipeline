...
Changes vs v4.31:
  - Per-event photon count vs baseline factor plot saved to
    event_plots/phot_vs_factor/{stem}_{ch}_event_{t}ms_phot.png.
    X-axis: BL_FACTORS (13 values); Y-axis: sum of baseline-subtracted
    intensity in the event window (no units); grey dashed line shows
    raw (unsubtracted) photon count as reference.
  - metadata.csv gains two new columns:
      raw_photon_count — sum(i100[t_left:t_right+1]), factor-independent
      photon_count     — sum(max(0, seg − factor×edge_bl)), one per row
...
PIPELINE_VERSION = 'v4.32'   # hardcoded — do not import from version.py
    ...
    for stem in FILES:
        ...
        for ch_id in meas.channel_ids:
            ...
            for ev in events:
                ...
                # v4.32: photon counts for all 13 factors (independent of D_hat validity)
                N_tr   = len(i100)
                ...
                for factor, (chosen, D_sim_val, pearson, sim_raw) in factor_results.items():
                    ...
                    all_meta_rows.append({
                        ...
                        'raw_photon_count':      raw_photon_count,       # v4.32
                        'photon_count':          phot_by_factor[factor], # v4.32
                    })
                    seen_events.add((stem, ch_id, t_peak))
                    n_plotted += 1

                # v4.32: photon count vs factor plot (one per event)
                phot_path = PHOT_DIR / f'{stem}_{ch_id}_event_{t_peak:07d}ms_phot.png'
                make_phot_plot(stem, ch_id, t_peak, phot_by_factor, raw_photon_count, phot_path)
