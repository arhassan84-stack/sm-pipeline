"""
Multi-scale CNN fusion on 90k traces + 283 features.

Three parallel CNN branches with kernel sizes 8, 32, 128 process the raw trace:
  k=8   → captures fine structure (short bursts, fast diffusion / d>=1)
  k=32  → captures medium-scale correlations
  k=128 → captures broad envelope (slow diffusion / d<1)
Each branch: 1→32→64→128 channels + AdaptiveAvgPool1d(1) → 128-dim
Feature branch: 283→256→128
Head: (3×128 + 128) = 512 → 256 → 64 → 1

Previously tested on 50k/207f (Round 2): MAPE=14.7%. Now retrained on 90k/283f.
Saves model_cnn_ms.pt for ensemble use.
"""
...
LABEL='pt_cnn_multiscale_90k'; BATCH_SIZE=256; EVAL_BATCH=512
...
class MS_Branch(nn.Module):
    """Single-scale CNN branch with configurable first kernel size."""
    def __init__(self, k):
        super().__init__()
    ...
    def forward(self, x): return self.net(x.unsqueeze(1)).squeeze(2)  # (B, 128)
...
class MultiScaleFusion(nn.Module):
    ...
    def forward(self, trace, feats):
        cnn_out = torch.cat([self.b8(trace), self.b32(trace), self.b128(trace)], dim=1)
        return self.head(torch.cat([cnn_out, self.feat_branch(feats)], dim=1)).squeeze(1)
...
log(f"Training {LABEL}  params={n_params:,}  (3 CNN branches: k=8,32,128 + feat branch)")
...
torch.save(model.state_dict(),'model_cnn_ms.pt')
np.save('scaler_cnn_ms_mean.npy',sc.mean_); np.save('scaler_cnn_ms_scale.npy',sc.scale_)
log("Saved model_cnn_ms.pt + scaler")
...
summary=(f"Task: {LABEL}\nArchitecture: Multi-scale CNN (k=8,32,128) + feature branch (283f) → fusion\n"
