"""
Specialist model for slow diffusion: trained on d <= 1.5 (90% split).
Overlaps with fast model in [0.5, 1.5] for smooth soft-gating.
...
DROPOUT      = 0.15
T0           = 50
D_THRESHOLD  = 1.5   # train on d <= this (generous overlap)
...
# Filter training to d <= D_THRESHOLD (with overlap), test to d <= 10
mask_tr = (d_train_all > 0) & (d_train_all <= D_THRESHOLD)
...
log(f"Train (d<={D_THRESHOLD}): {len(d_train)}  Test (d<=10): {len(d_test)}")
...
torch.save(model.state_dict(), 'model_specialist_slow.pt')
np.save('scaler_specialist_slow_mean.npy', scaler.mean_)
np.save('scaler_specialist_slow_scale.npy', scaler.scale_)
log("Saved model_specialist_slow.pt + scaler")
...
summary = (f"Task: {LABEL}\nArchitecture: 2048-1024-512-256-128 + BN + Dropout({DROPOUT})\n"
           f"Training filter: d <= {D_THRESHOLD}  Train samples: {len(d_train)}\n"
...
ax=axes[0]; ax.scatter(d_test,d_pred_te,alpha=0.2,s=5,c=np.where(d_test<1,'steelblue','darkorange'),label='d<1 / d>=1')
