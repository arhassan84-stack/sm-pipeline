"""
stage3_v3_95.py — Stage 3 event detection pipeline, version 3.95
═══════════════════════════════════════════════════════════
...
def _compute_w12(signal: np.ndarray, tp: int,
    ...
    Returns
    -------
    (w12, bl_min) — half-width in ms bins, and the minimum baseline value used
        for I_cut computation (None when baseline is None).
    """
...
def _edge_baseline(bundle: TraceBundle, t_left: int, t_right: int) -> float:
    """Minimum of bundle.baseline at the left and right edges of a window.

    Using the minimum rather than the mean prevents a noise-model baseline
    that is contaminated by a slow transit from raising the subtraction level
    above the true pre-event background.  The contaminated edge is typically
    the one that falls inside (or just past) the transit region, where the
    KDE-mode estimator is pulled up by the transit's own photons.  Taking the
    minimum always selects the uncontaminated edge and preserves the full
    shape of the transit in the model input, avoiding the artificial truncation
    of the lower flanks that leads to erroneous fast-D predictions.
    """
...
def _step1(bundle: TraceBundle,
    ...
    owner1 : int32 (N,) array — Cat-1 bin ownership; -1=unclaimed, else=event id
    claimed2 : bool (N,) array — Cat-2 seed bins marked True
    """
    N        = len(bundle.i1ms)
    signal   = bundle.i1ms
    owner1   = np.full(N, -1, dtype=np.int32)  # Cat-1 ownership (int, -1=unclaimed)
...
def _step2(bundle: TraceBundle,
    ...
    claimed_self  : for Cat-2 (allow_swallow=False) — bool (N,) array; for
                    Cat-1 (allow_swallow=True) — int32 (N,) owner array where
                    -1=unclaimed and a non-negative value is the event id of the
                    owner.  Updated in-place as windows expand.  Expansion stops
                    when it would enter territory already owned by a FINALISED
                    event of the same category.  Pending (not yet expanded)
                    Cat-1 events can be swallowed: their seed bins are unclaimed
                    and the expanding event absorbs their territory.
    d_model       : ModelWrapper — loaded D-inference model
    category      : 1 or 2 — passed through to output dicts
    allow_swallow : if True, enable Cat-1 swallowing of pending lower-amplitude
                    Cat-1 events during expansion.

    Returns
    -------
    list of event dicts — includes both finalised events and swallowed events
    (swallowed events carry a 'swallowed_by' key with the tp of the absorber).
    """
    N       = len(bundle.i1ms)
    results = []
    ...
    # Swallowing state (Cat-1 only)
    if allow_swallow:
        finalized_ids: set = set()   # events that have completed expansion
        swallowed_ids: set = set()   # events absorbed by a higher-amplitude neighbour
    ...
    for i, (ev, D_hat_init) in enumerate(zip(seed_events, initial_d_hats)):
        ...
        # ── Swallowed event: record seed state and skip expansion ──────────
        ...
        # a FINALISED Cat-1 event (or W_MAX_MS) stops the expansion.
        # Lower-amplitude pending Cat-1 events are swallowed instead.
        ...
        if not fast_group:
            ...
            while w < W_MAX_MS:
                ...
                # Stop 1a: W_MAX reached
                if w_new >= W_MAX_MS:
                    break

                # Stop 1b: collision check — behaviour differs by category
                if allow_swallow:
                    # Cat-1: identify owners in the proposed new ranges
                    ...
                    if owners_in_range:
                        if any(o in finalized_ids for o in owners_in_range):
                            # Blocked by a finalised event — cannot expand further
                            break
                        # All blocking events are still pending: swallow them
                        for swallowee_id in owners_in_range:
                            swallowee = ev_by_id[swallowee_id]
                            # Unclaim the swallowee's entire seed window
                        ...
                        # Fall through — expansion proceeds after swallowing
                else:
                    # Cat-2: simple bool collision check
...
def run_stage3_v2(bundle: TraceBundle, d_model, radius_cap: int = 128,
    ...
    # Step 1 — peak finding + claiming
    # Returns owner1 (int32 array, -1=unclaimed) for Cat-1 and a bool array
    # for Cat-2 so each category's expansion only blocks events of the same
    # category (Cat-1 ↔ Cat-2 overlap is permitted).
    ...
    # Cat-1: allow_swallow=True — a higher-amplitude Cat-1 event can absorb
    # lower-amplitude (pending) Cat-1 seeds that fall in its expansion path.
    # Swallowed events are returned in the list with a 'swallowed_by' field.
    ...
    # Separate swallowed Cat-1 events so they bypass the post-absorption merge
    # (they are already accounted for) but remain in the final output.
