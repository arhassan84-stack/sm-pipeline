...
Changes vs v4.4:
  - AUC-based factor selection: for each event and each factor, the
    normalized area under the curve (peak-normalized integral) of the
    baseline-subtracted real trace is compared to that of the
    Pearson-selected sim replica.  The factor minimising
        |nAUC_real − nAUC_sim|
    is recorded as auc_best_factor.
  - auc_opt/ subfolder: best-AUC event plot copied with 'g_' prefix.
  - Per-measurement + grand-summary AUC factor frequency bar charts
    saved to auc_opt/.
  - auc_best_factor column added to metadata.csv.
  - Comparison histogram and window-size scatter updated with a third
    AUC-opt series (mediumseagreen).
...
PIPELINE_VERSION = 'v4.5'   # hardcoded — do not import from version.py
...
for d in [OUT_DIR, SIM_DIR, EVT_DIR, OPT_DIR, PHOT_DIR, KNEE_OPT_DIR, AUC_OPT_DIR]:
...
def make_comparison_d_histogram(stem, ch_id, d_pearson, d_knee, d_auc, out_path):
    """Overlapping D̂ histograms on a log x-axis: Pearson-opt vs Knee-opt vs AUC-opt."""
    ...
    if len(d_p) == 0 and len(d_k) == 0 and len(d_a) == 0:
        return
    ...
    ax.set_xlabel('D̂  (µm²/s)', fontsize=FS_LABEL, color='black')
    ax.set_ylabel('Events',      fontsize=FS_LABEL, color='black')
    ax.set_title(f'{stem} / {ch_id}  —  Pearson vs Knee vs AUC D̂ distribution',
                 fontsize=FS_TITLE, color='black')
...
def make_scatter_window_d(stem, ch_id,
                          ...
                          ws_auc,     d_auc,
                          out_path):
    """Scatter: window size (ms) vs D̂ (µm²/s) — Pearson-opt vs Knee-opt vs AUC-opt."""
...
try:
    ...
    for stem in FILES:
        ...
        for ch_id in meas.channel_ids:
            ...
            ws_auc_list     = []   # window size (ms) for AUC-opt events
            ...
            for ev in events:
                ...
                for factor, (chosen, D_sim_val, pearson, sim_raw) in factor_results.items():
                    ...
                    all_meta_rows.append({
                        ...
                        'auc_best_factor':       auc_best_factor,  # v4.5
                    })
            ...
            # v4.5: Pearson vs Knee vs AUC comparison plots
            if n_plotted > 0:
                make_comparison_d_histogram(
                    stem, ch_id, d_pearson_list, d_knee_list, d_auc_list,
            ...
            # v4.33/v4.5: per-measurement frequency bar charts
            if n_plotted > 0:
    ...
    # v4.33/v4.5: grand-summary frequency bar charts (one per channel across all measurements)
