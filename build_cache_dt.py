"""
Build train/test feature caches for dt-specific FCS simulations.

Loads raw simulation output (from generate_simulations_dt.py), computes the
full 302-feature pipeline, and saves a 90/10 random train/test split.

This is a STANDALONE cache builder — it does NOT merge with the existing
90pct or aug caches.  Each dt gets its own independent train/test pair.

Output files (using --dt_tag, e.g. dt025):
  cache_i_train_dt025.npy   (N_train, 4096)  float32 raw traces
  cache_X_train_dt025.npy   (N_train, 302)   float64 features
  cache_d_train_dt025.npy   (N_train,)       float64 diffusion coefficients
  cache_i_test_dt025.npy    (N_test,  4096)  float32
  cache_X_test_dt025.npy    (N_test,  302)   float64
  cache_d_test_dt025.npy    (N_test,)        float64

Usage:
  python build_cache_dt.py \\
      --dt_tag dt025 \\
      --i_file sims_dt025_noise0_i.npy \\
      --d_file sims_dt025_noise0_d.npy \\
      --workers 8 --chunk-size 10000
"""
import numpy as np
import argparse
import time
...
# ── Fixed pipeline constants (independent of trace length N) ─────────────────
...
def make_lag_constants(N):
    """Compute lag-related constants from trace length N."""
...
# ── Feature pipeline (parameterised by N = trace length) ─────────────────────
...
def _feature_chunk(args):
    """Compute full 302-feature pipeline for one chunk of traces."""
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Build dt-specific feature caches')
    parser.add_argument('--dt_tag', required=True,
                        help='Tag identifying the dt (e.g. dt025, dt050)')
    parser.add_argument('--i_file', required=True,
                        help='Intensity traces .npy  (output of generate_simulations_dt.py)')
    parser.add_argument('--d_file', required=True,
                        help='D-value labels .npy    (output of generate_simulations_dt.py)')
    parser.add_argument('--workers', type=int, default=cpu_count(),
                        help='Parallel workers for feature extraction (default: all CPUs)')
    parser.add_argument('--chunk-size', type=int, default=10000,
                        help='Traces per sub-task (default: 10000)')
    parser.add_argument('--test-frac', type=float, default=0.10,
                        help='Fraction held out as test set (default: 0.10)')
    ...
    log(f"Loading simulation files for {tag}...")
    i_all = np.load(args.i_file)   # (N_sims, N_BINS) float32
    d_all = np.load(args.d_file)   # (N_sims,)
    N_BINS = i_all.shape[1]        # derive trace length from data
    log(f"  Loaded: {i_all.shape}  trace length: {N_BINS} bins")
    log(f"  d range: [{d_all.min():.4f}, {d_all.max():.4f}]")

    # Compute lag constants from actual trace length
    ...
    i_train, d_train = i_all[train_idx], d_all[train_idx]
    i_test,  d_test  = i_all[test_idx],  d_all[test_idx]
    del i_all, d_all
    log(f"  Train: {len(d_train)}  Test: {len(d_test)}")

    # Compute features — train
    n_workers = min(args.workers, len(i_train))
    n_chunks  = max(n_workers, int(np.ceil(len(i_train) / args.chunk_size)))
    chunks    = np.array_split(i_train, n_chunks)
    args_list = [(c, N_BINS, LAGS_CURR, LAGS_ARR, NEW_SHORT_LAGS) for c in chunks]
    log(f"Computing train features ({len(i_train)} traces, {n_workers} workers, {n_chunks} chunks)...")
    t_f = time.time()
    with Pool(n_workers) as pool:
        results = pool.map(_feature_chunk, args_list)
    X_train = np.vstack(results)
    log(f"  Train features: {X_train.shape}  ({time.time()-t_f:.1f}s)")

    # Compute features — test
    n_w_te    = min(args.workers, len(i_test))
    n_ch_te   = max(n_w_te, int(np.ceil(len(i_test) / args.chunk_size)))
    chunks_te = np.array_split(i_test, n_ch_te)
    args_te   = [(c, N_BINS, LAGS_CURR, LAGS_ARR, NEW_SHORT_LAGS) for c in chunks_te]
    log(f"Computing test features ({len(i_test)} traces, {n_w_te} workers, {n_ch_te} chunks)...")
    t_f2 = time.time()
    with Pool(n_w_te) as pool:
        results_te = pool.map(_feature_chunk, args_te)
    X_test = np.vstack(results_te)
    log(f"  Test features: {X_test.shape}  ({time.time()-t_f2:.1f}s)")

    # Save caches
    log(f"Saving cache_{{i,X,d}}_{{train,test}}_{tag}.npy ...")
    np.save(f'cache_i_train_{tag}.npy', i_train)
    np.save(f'cache_X_train_{tag}.npy', X_train)
    np.save(f'cache_d_train_{tag}.npy', d_train)
    np.save(f'cache_i_test_{tag}.npy',  i_test)
    np.save(f'cache_X_test_{tag}.npy',  X_test)
    np.save(f'cache_d_test_{tag}.npy',  d_test)
    log(f"Done. Total: {time.time()-t0:.1f}s")
    log(f"  Train: {i_train.shape}  features: {X_train.shape}")
    log(f"  Test:  {i_test.shape}   features: {X_test.shape}")
