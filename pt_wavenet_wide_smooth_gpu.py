"""
WaveNet1D(channels=256) trained on smoothed FCS traces.

Takes --window W and loads the corresponding pre-built smoothed caches:
  cache_i_train_smooth{W}.npy / cache_X_train_smooth{W}.npy / cache_d_train_smooth{W}.npy
  cache_i_test_smooth{W}.npy  / cache_X_test_smooth{W}.npy  / cache_d_test_smooth{W}.npy

Architecture is identical to wavenet_wide_aug (channels=256, same hyper-params).
Both training and test traces are pre-smoothed, so the model learns to predict
D from denoised traces.  Compare across window sizes to assess smoothing effect.

Saves:
  model_wavenet_wide_smooth{W}.pt
  scaler_wavenet_wide_smooth{W}_mean/scale.npy
  pred_logd_wavenet_wide_smooth{W}_test.npy
  results_pt_wavenet_wide_smooth{W}.txt

Usage:
  python pt_wavenet_wide_smooth_gpu.py --window 5
"""
import argparse
...
parser.add_argument('--window', type=int, required=True,
                    help='Smoothing window size (must match a built cache)')
args = parser.parse_args()
W = args.window

LABEL        = f'pt_wavenet_wide_smooth{W}'
CHANNELS     = 256
BATCH_SIZE   = 128
EVAL_BATCH   = 256
MAX_EPOCHS   = 200
...
t_total = time.time()
log(f"Device: {device}  Window: W={W}")

# ── Load smoothed caches ──────────────────────────────────────────────────────
log(f"Loading smoothed caches (W={W})...")
i_train = np.load(f'cache_i_train_smooth{W}.npy').astype(np.float32)
X_train = np.load(f'cache_X_train_smooth{W}.npy').astype(np.float32)
d_train = np.load(f'cache_d_train_smooth{W}.npy')
i_test  = np.load(f'cache_i_test_smooth{W}.npy').astype(np.float32)
X_test  = np.load(f'cache_X_test_smooth{W}.npy').astype(np.float32)
d_test  = np.load(f'cache_d_test_smooth{W}.npy')

# Apply d<=10 mask (same as all other models)
mask_tr = d_train <= 10
...
log(f"Train (d<=10): {len(d_train)}  Test: {len(d_test)}")

# ── Normalise traces (per-trace z-score, same as all wavenet models) ──────────
...
# ── Save model + scaler ───────────────────────────────────────────────────────
torch.save(model.state_dict(), f'model_wavenet_wide_smooth{W}.pt')
np.save(f'scaler_wavenet_wide_smooth{W}_mean.npy',  feat_scaler.mean_)
np.save(f'scaler_wavenet_wide_smooth{W}_scale.npy', feat_scaler.scale_)
log(f"Saved model_wavenet_wide_smooth{W}.pt + scaler")
...
np.save(f'pred_logd_wavenet_wide_smooth{W}_test.npy', logd_pred_te)
log(f"Saved pred_logd_wavenet_wide_smooth{W}_test.npy")
...
summary = (
    f"Task: {LABEL}\n"
    f"Smoothing window: W={W} (movmean, nearest-pad)\n"
...
print(f"\n{'='*60}\n{summary}{'='*60}")
with open(f'results_pt_wavenet_wide_smooth{W}.txt', 'w') as f:
    f.write(summary)
log(f"Saved results_pt_wavenet_wide_smooth{W}.txt"); print("Done.")
